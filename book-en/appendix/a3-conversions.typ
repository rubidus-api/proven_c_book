...
A compression of chapter 29's rules into tables.
...
That is why `-1 < 1u` is false — `-1` turns into a huge unsigned value (chapter 29).
...
#dtable(
  ...
  [`void *` → another pointer], [lossless], [outside the contract if the alignment does not match (chapter 37)],
...
- *Array → pointer*: in most contexts it decays into the address of the first element.
  ...
  initialising an array from a string literal (chapters 38 and 40). `alignof` takes only
