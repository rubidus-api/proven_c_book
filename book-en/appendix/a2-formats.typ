...
Chapter 22 learned the minimal set (`%d`, `%s`, `%%`) and chapter 60 dissected the
...
is outside the contract (undefined behaviour) (chapters 46 and 60).
...
separately (chapters 57 and 88's views) can be printed as it is, without NUL
termination.
...
the variadic argument's type goes out of step — the very mismatch seen in chapter 57.
...
Type information does not ride along into variadic arguments (chapter 57), so this
...
promotion, so `%f` must be `float *` and `%lf` `double *` without fail (chapter 60's
misconception box).
...
- *A failed argument is not touched.* Use it without checking and you mistake the
  previous value for new input (chapter 83's counterexample is that).
...
- *It does not report overflow.* Giving `99999999999` to a `%d` is outside the
  contract. If a range check is needed, use the `strtol` family or chapter 89's
  scanner.
...
`need >= (int)sizeof buf` (chapter 83).
...
#dtable(
  ...
  [`printf(user)`], [the format string vulnerability (chapter 60)], [`printf("%s", user)`],
