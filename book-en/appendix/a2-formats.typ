...
separately (chapters 53 and 78's views) can be printed as it is, without NUL
termination.
...
- *A failed argument is not touched.* Use it without checking and you mistake the
  previous value for new input (chapter 73's counterexample is that).
...
- *It does not report overflow.* Giving `99999999999` to a `%d` is outside the
  contract. If a range check is needed, use the `strtol` family or chapter 79's
  scanner.
...
`need >= (int)sizeof buf` (chapter 73).
