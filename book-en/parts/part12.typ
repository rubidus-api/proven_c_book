...
proved itself (chapter 41).

So this part too begins not with the tool but with the *problem*. Chapter 82
...
point to the answer for each — installation and a first program (chapter 83), errors
that come as values (chapter 84), bytes and views (chapter 85), allocators
(chapter 86), strings (chapter 87), formatting and parsing (chapter 88), containers
and algorithms (chapter 89), files, time and random numbers (chapter 90), and
running things overlapped and environments with no OS (chapter 91).
...
implementation has not yet (chapter 91 sets down the supported range and the
stability honestly).
