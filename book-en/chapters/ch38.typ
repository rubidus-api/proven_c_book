...
#prereq(
  ...
  ([chapter 37, Arrays], [arrays and subscripts]),
)
...
#deepqa[
  ...
  Now that you have learned arrays (chapter 37) — how would you define a C string
  exactly, yourself?
][
...
not as a type*, and breaking it is chapter 48's undefined behaviour — mostly it dies on
...
#qa[
  ...
  *So as not to break existing code.* The word `const` itself entered the standard only
  ...
  It is the same circumstance as `gets`'s funeral taking twenty years (chapter 58) —
...
#demo("examples-en/ch38/literal.c")
...
into a pointer is the decay seen in chapter 37, and `sizeof` being the exception to
...
And recalling the identity `a[i] ≡ *(a + i)` learned in chapter 37, since addition
...
in the preprocessor but in *translation phase 6* (chapter 51's table) — so a string
...
with literals the compiler predefines (`__DATE__`, chapter 51), so many programs build
...
The demand that a format string *be a constant* (chapter 55's format string
...
seen in chapter 65 and appendix B stand exactly on this rule.
...
them at the place of use.* And all three pair with chapter 51's `#` operator. If the
...
#define VER_MINOR 1
#define STR_RAW(x) #x
#define STR(x)     STR_RAW(x)                     /* double expansion (chapter 51) */
...
where chapter 61's "the size of `wchar_t` differs by implementation" shows itself at
...
#realcase[
  ...
  with the boundary violation of chapter 37 built into its design. In 1988 the
...
#qa[
  ...
  *character layer* (counting characters, cutting, case conversion) requires UTF-8
  ...
  met in the next chapter, in earnest in chapter 55). Keep one rule in mind and
