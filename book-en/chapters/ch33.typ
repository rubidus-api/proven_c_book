...
- *Is a struct copied whole when passed?* — yes (chapter 45). Which is why the
...
(chapter 51). There is no rule to memorise, only a pattern to remember: *change
...
#qa[
  ...
  passing pointers (treated with structs in chapter 45). But there are two
...
smaller" (searching tree structures is the representative — chapter 45 onwards),
...
calls", whose identity (the stack) is met in chapter 43.
...
— make a type holding "one of several types" with chapter 47's tagged union, or
choose the branch at compile time with chapter 57's `_Generic`.
...
#dtable(
  ...
  [memory], [`& * [] . ->`], [address, dereference, subscript, member (chapters 31, 35 and 45)],
...
(chapter 50) and of proven's design philosophy (chapter 42), and it is faced head
on in Part IX.
