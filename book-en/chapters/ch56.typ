...
#prereq(
  ...
  ([chapter 29, Implicit conversions], [the default argument promotions]),
)

#deepqa[
#idx("variadic arguments")  Chapter 29 taught that values crossing into variadic
...
int when a double really arrived — and it is outside the contract (chapter 50).
...
chapter 29 really happens here.
...
(chapter 59), and why writing `va_arg(ap, float)` is a contract violation, taking
...
value of the original type, otherwise `unsigned int`" (chapter 29). On today's
...
widths are out of step. That is why C23's `nullptr` (chapter 36) or an explicit
cast (`(char *)0`) is used.
...
exist. That is why chapter 82 counts this among "the five bugs C has been
...
#demo("examples-en/ch56/generic.c")
...
The `proven_println("{}", PROVEN_ARG(x))` used in chapter 88 is exactly this
...
value itself (exactly chapter 46's tagged union).
...
#qa[
  ...
  stamps code out with macros (chapter 89's container macros) or passes `void *`
  with a size (chapter 82's `qsort`).
]
...
#misconception[
  ...
  if there are many, take an array and a length (chapter 38's practice); if the
  kinds vary, take an explicit list of values with chapter 46's tagged union. The
