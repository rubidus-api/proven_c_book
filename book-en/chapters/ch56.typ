...
#demo("examples-en/ch56/parens.c")
...
only the precedence problem, not this one. It is the place where chapter 33's "one
...
and left only a trailing comma (that comma being legal thanks to chapter 45's
initialiser syntax).
...
textbook case of *unspecified behaviour*, one of the grey zones learned in chapter 51 —
...
The `__VA_OPT__` C23 brought in (chapter 80) puts something in "only when the variadic
...
#demo("examples-en/ch56/xmacro.c")
...
Moreover it uses the *designated initialiser* `[code] = #name` (chapter 45) so
...
#qa[
  ...
  (chapter 53). The macro line continuations (`\\`) disappear and the list reads
