...
#prereq(
  ([chapter 35, Objects, addresses, pointers], [what a pointer is]),
...
#deepqa[
  ...
  address through int's eye" (chapter 35), and if that address breaks int's
...
#demo("examples-en/ch37/align.c")

=== The two privileges side by side

Here we tidy up the two passages announced in chapter 35. They do different work.
...
representation is for exactly this combination (chapter 35's word-addressed
machines).
...
- *Pointer arithmetic stays inside the object it was born in.* Taking a pointer
  that started from one object's address and pushing it (chapter 38) onto another
...
- *One past the end is permitted.* Pointing at the slot "after" the end of an
  ...
  specially (in action in chapter 38).
