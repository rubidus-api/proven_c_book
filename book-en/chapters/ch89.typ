...
#prereq(
  ([chapter 57, Variadic functions], [variadic arguments and the format string]),
  ([chapter 60, The terrain of the standard library], [the printf contract]),
)

#deepqa[
  Chapter 57 said type information does not ride along into variadic arguments, and
  chapter 60 said that is why the format string alone settles "how the stack is to be
...
#organizer[
  The answer to chapter 83's third bug — the mismatch of format and argument. How the
  ...
  alternative to the `printf` and `scanf` taken apart in chapter 60.
]
...
The possibility of mismatch seen in chapter 60 is removed at the level of syntax.
...
They correspond to chapter 60's `printf` formats but differ in three ways. *The
...
`-0000042`, not `000000-42` — the same rule as `%08d` seen in chapter 60.
...
#demo("examples-en/ch89/fmt.c")

This example formats not to the screen but *into a string* — the `proven_println`
seen in chapter 84 is the edition connecting this machinery to standard output. Three
things can be pointed out.

*First, formatting too can fail.* `example.org:8080` cannot go into an 8-byte vessel,
so it was refused. Chapter 88's principle stands here too — rather than truncate, it
...
correspond to chapter 60's `%10s`, `%-10s` and `%.3f`, differing in that *there is no
type letter*.
...
`required` is the same information as `snprintf`'s return value seen in chapter 60. The
...
#qa[
  ...
  exact opposite of chapter 60's `printf`, which accepted anything.
]
...
#misconception[
  ...
  `%f` seen in chapter 60 is the more awkward task.
]
...
differs from expectation*. And as seen in chapter 60, `sscanf` tells only "how many
...
That `proven_scan_str` *returns a view without copying* matters — chapter 88's "text
handling without copying" holds in parsing too. In exchange that view is valid only
while the original input is alive (chapter 86).

#demo("examples-en/ch89/lines.c")
...
#qa[
  ...
  chapter 85 is not guaranteed here, so if it is really needed the cursor and the
...
#realcase[
  ...
  chapter 88's story of encodings — *do not read ambiguous input as something mended;
...
algorithms with "a guarantee even in the worst case" foretold in chapter 83.
