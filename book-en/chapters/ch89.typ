...
#prereq(
  ([chapter 44, Structs], [structs]),
  ([chapter 43, Dynamic memory], [a growing array]),
  ([chapter 85, The foundation], [views and bounds]),
)

#deepqa[
  Chapter 37 taught that a C array's size is settled at compile time, and chapter 43
  ...
  wrap-round seen in chapter 85 happens here. Second, *the state on failure* — when
...
#idx("hash map")  The tools that hold many — the growing array, the intrusive list,
  the ring buffer, the hash map. And as the answer to chapter 82's fifth bug
  ...
  Chapter 37's arrays and chapter 44's structs become practical components here.
]
...
especially loved in embedded work (chapter 91).
...
allocator per operation (chapter 87), while an array takes one at creation and keeps it
...
itself — and that growth happens through the allocator (exactly chapter 86's rule; the
...
#antipattern[
  ...
  *first = 7;                               /* writes at the old address — use after free */
  ...
  chapter 43 appears on a container. The rule is one — *after changing a container,
...
`proven_list_node_t link;` slot inside a `struct` made in chapter 44.
...
environment with no heap (chapter 91), and hanging the same object on two lists at
...
`link` member — the `offsetof` seen in chapter 44 used here in the flesh. This one line
...
and *an owned key* (the map copies and holds it). Chapter 86's owning-borrowing
...
that "absent" is not a failure but a normal answer (distinct from chapter 84's error
...
#demo("examples-en/ch89/wordcount.c")

This one example contains all of this chapter's tools — it counts with a map, gathers
into an array, sorts and prints. Chapter 87's views were used to cut the words, so
...
#qa[
  ...
  randomness — and in an environment with no OS (chapter 91) there is nowhere to draw
...
The two problems seen in chapter 82 — the unchecked comparator, and the algorithm
...
complexity attack seen in chapter 82 does not work. To carry the header's wording
...
by count, and *ties broken by the word*. It contrasts exactly with chapter 82's
...
- *hex and Base64* — two standards for moving bytes into text. The principle here is
  ...
  mended* but refused with `PROVEN_ERR_INVALID_ENCODING` (that norm from chapter 87).
