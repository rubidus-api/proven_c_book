...
argument too.* What `fgetc` returns must be passable straight in (chapter 60), and
...
the first trap. Chapter 60's rule, "always receive what `fgetc` returns in an
`int`", comes back here.
...
characters* and use `<wctype.h>`'s `iswalpha` (chapters 67 and 68), or *work on
the byte string as it is* and do only the judgements you need yourself
(chapter 69). This book recommends the latter, and chapter 69 says why.
