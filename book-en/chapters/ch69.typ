...
#prereq(
  ([chapter 66, What the new standards added, and the `*_s` controversy], [what the standards added]),
  ([chapter 29, Booleans and comparison], [the type of true and false]),
)

#deepqa[
  Chapter 66 passed over in a table only that C23 made `<stdbool.h>` effectively
...
#demo("examples-en/ch69/keywords.c")
...
for avoiding the problem of sharing seen in chapter 67 *by not sharing*. `errno` is
...
#realcase[
  ...
  `_Alignas`, `_Atomic` (chapter 67) and `_Generic` are all products of this rule.
...
on in an operating system and in an embedded chip respectively (chapter 70), and
what an allocator actually does in the heap region of that map (chapter 71). Those
