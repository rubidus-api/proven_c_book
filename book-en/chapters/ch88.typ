...
#prereq(
  ([chapter 41, Strings], [strings]),
  ([chapter 9, Characters and text], [UTF-8 and bytes]),
)

#deepqa[
  Chapter 41 said that for a C string "up to the NUL" is the length, so to know the
...
#organizer[
  The answer to chapter 83's first bug — string functions that do not know the size
  ...
  truncate*. Chapter 9's story of encodings and chapter 41's story of boundaries
...
The rule set up in chapter 87 applied to strings as it is. `u8` means UTF-8 — that
...
```c
/* the borrowed string — the same shape as chapter 86's mem_view_t */
typedef struct {
...
#demo("examples-en/ch88/ops.c")

The first line shows chapter 87's rule again. `proven_u8str_borrow` has no allocator —
...
It is the exact opposite of `snprintf`'s choice seen in chapter 83. Why not truncate?
...
#qa[
  ...
  name (the same as chapter 86's `_unchecked`) is kept here too.
]
...
`_reserve`. As seen in chapter 87, `_reserve` pays especially on an arena — take it
...
kept — as seen in chapter 86, *it is valid only while the original is alive*.
...
Chapter 83 spoke of the danger of sentinel values, and what differs here is that *it
...
the place where chapter 83's third bug waits, and the place where this library's most
