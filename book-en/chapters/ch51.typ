...
#prereq(
  ...
  ([chapter 50, Errors and contracts], [what it means to break a contract]),
)
...
#idx("undefined behavior")  implementation-defined, unspecified, undefined behavior — are *the standard's own
...
Why should such a thing be UB? Recall the *translation phases* seen in chapter 56 and
...
#dtable(
  ...
  [`int _Value;`, `int __x;`], [trespassing on the reserved name space (chapter 80)], [7.1.3],
...
#demo("examples-en/ch51/checked.c")
...
governed by the discipline learned in chapter 50 ("errors are values").

*Components* — using an API in which violating the contract is difficult to begin
with (chapter 42's proven is that layer). Chapter 17's metaphor — tools are nets,
...
The part of precision is over — how to handle approximation (chapter 49), how to
handle failure (chapter 50), and how to know the world outside the contract
(chapter 51). The three chapters share one theme: *C is a language that entrusts
...
into several files (chapter 53), we face the layer of preprocessing and
translation (chapter 56), we learn the terrain of the standard library
(chapter 60), we treat proven head on (Part XII), and we close the book with the
practices of modern C (chapter 96).
