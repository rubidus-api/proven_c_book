...
chapter 40's rule that array parameters decay into pointers). The names are free
...
environment variables is `getenv` (chapter 61).
...
#antipattern[
  ...
  implementation* (chapter 59) the name and form of the starting function are the
...
#qa[
  ...
  *Write it yourself* — for a short program this is enough. Scan `argv`,
  ...
  slot. To convert to a number use `strtol`, not `atoi` (chapter 61).
  ...
  *A library* — as the scale grows (subcommands, generated help, merging with a
  ...
  Whichever road, keep one rule: *arguments are input from outside.* Chapter 42's
...
#platform[
  ...
  *Windows* — `CreateProcess` passes *one string* (chapter 3). That is, the dividing
  ...
  rules) — the same grain as chapter 64's `system` counterexample.
]
...
after `main` ended. Flushing open streams (chapter 61's buffers) is included in
...
#platform[
  ...
  as chapter 59 showed — the function the reset vector points at is the starting
...
#qa[
  ...
  *the C program's point of view*. In reality, startup code (something like `crt0`)
  ...
  calling `main` (chapter 80). `main` is not "the first code that runs" but "the
