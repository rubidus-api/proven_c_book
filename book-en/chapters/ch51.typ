...
#demo("examples-en/ch51/parens.c")
...
and left only a trailing comma (that comma being legal thanks to chapter 42's
initialiser syntax).
...
textbook case of *unspecified behaviour*, one of the grey zones learned in chapter 48 —
...
The `__VA_OPT__` C23 brought in (chapter 68) puts something in "only when the variadic
...
#demo("examples-en/ch51/xmacro.c")
...
Moreover it uses the *designated initialiser* `[code] = #name` (chapter 42) so
...
#qa[
  ...
  (chapter 50). The macro line continuations (`\\`) disappear and the list reads
