...
Chapter 33 taught that `void *` is "a vessel that holds any data pointer". Yet
...
table, not the code, and combined with chapter 49's X macro you can even generate
...
+ *The base struct is the first member.* The standard guarantees that "a struct's
  ...
  `struct circle *` and `struct shape *` can be safely gone between (chapter 41).
