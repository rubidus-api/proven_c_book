...
#prereq(
  ...
  ([Chapter 45, Structures], [member access]),
)
...
#deepqa[
  ...
  chapter 28, comparisons in chapter 30, assignment in chapter 34, pointer
  arithmetic in chapter 38. So what is left to learn here?
][
...
#dtable(
  ...
  [Side effects], [Whether it changes an object or the outside world (chapter 34)],
)

*Value category* sounds like jargon, but you have been using it all along.
What may appear on the left of an assignment is an lvalue (chapter 34), and
what you may apply `&` to is an lvalue (chapter 35). An array name is an
lvalue that nonetheless cannot be assigned to — a special case (chapter 38).
...
and the rule that settles an integer constant's type is in chapter 27. This chapter
...
#qa[
  ...
  of the `for` statement, not of the postfix operator (chapter 32). Since nobody uses
...
#qa[
  ...
  - *And the one real rule --- never touch the same object twice in one expression.*
    Prefix or postfix, keep that and this operator will not hurt you. Chapter 33's
...
- *Grey zone* — in the three words of chapter 51. *UB* is undefined
...
#dtable(
  ...
  [`a[i]` subscript], [one a pointer to a complete object type, the other an integer], [an lvalue of the pointed-at type. `a[i]` is `*(a+i)`], [*UB*: access outside the array (including following the one-past-the-end position)], [chapter 38],
  [`f(...)` call], [a function, or a pointer to one], [the function's return type; not an lvalue], [*unspecified*: the order in which arguments are evaluated. *UB*: arguments that disagree with the prototype], [chapters 21, 24, 57],
  [`s.m` member], [a struct or union value and a member name], [the member's type; an lvalue if the left side is one], [*UB*: reading a union member other than the one last written (the common initial sequence is an exception)], [chapters 45, 47],
  [`p->m` member], [a pointer to a struct or union, and a member name], [the member's type, an lvalue], [*UB*: a null or otherwise invalid pointer], [chapter 45],
  [`x++` post-increment], [a modifiable lvalue of real or pointer type], [*the value before the change*; not an lvalue], [*UB*: two modifications within one sequence point; signed integer overflow], [chapter 32],
  [`x--` post-decrement], [the same], [the value before the change], [the same], [chapter 32],
)
...
#dtable(
  columns: 5,
  [*operator*], [*operands*], [*result*], [*grey zone*], [*in detail*],
  [`++x` `--x`], [a modifiable lvalue of real or pointer type], [*the value after the change*], [`++E` is `(E += 1)` — the overflow rules are the same], [chapter 32],
  [`&x` address-of], [a function designator, the result of `[]` or unary `*`, or an lvalue that is *not a bit-field and not declared `register`*], [a pointer to it], [breaking the constraint is a compile error], [chapter 35],
  [`*p` indirection], [a pointer type], [an lvalue of the pointed-at type], [*UB*: null, an object whose lifetime has ended, a misaligned address, or one outside its provenance], [chapters 35, 37, 43],
  [`+x`], [arithmetic type], [the promoted value], [], [chapters 20, 29],
  [`-x`], [arithmetic type], [the promoted value], [*UB*: signed integer overflow (`-INT_MIN`)], [chapter 27],
  [`~x`], [integer type], [the bitwise complement after promotion], [it happens at the promoted width — mind narrow types], [chapter 28],
  [`!x`], [scalar (arithmetic or pointer)], [`0` or `1`, of type `int`], [], [chapter 30],
  [`(type)x` cast], [between scalars], [a value of that type; not an lvalue], [*implementation-defined*: pointer↔integer conversion. *UB*: following a misaligned pointer; converting between function and object pointers], [chapters 29, 37],
  [`sizeof`], [a complete object type or an expression. *Not a function type, an incomplete type, or a bit-field*], [a `size_t` value], [with a variable length array the operand *is* evaluated at run time; otherwise it is not evaluated], [chapters 35, 38],
  [`alignof`], [the *name* of a complete object type (not an expression)], [a `size_t` value], [], [chapter 37],
)
...
#dtable(
  columns: 5,
  [*operator*], [*operands*], [*result*], [*grey zone*], [*in detail*],
  [`*` multiply], [arithmetic types], [the common type of the usual arithmetic conversions], [*UB*: signed integer overflow], [chapters 27, 29],
  [`/` divide], [arithmetic types], [the same], [*UB*: a zero divisor, `INT_MIN / -1`], [chapters 28, 49],
  [`%` remainder], [*integer types only*], [the same], [*UB*: a zero divisor, `INT_MIN % -1`], [chapter 28],
  [`+` add], [both arithmetic, or a pointer to a complete object type and an integer], [the common type, or the pointer type], [*UB*: integer overflow; pointer arithmetic beyond the array], [chapters 27, 38],
  [`-` subtract], [both arithmetic, a pointer and an integer, or *two pointers into the same array*], [`ptrdiff_t` for pointer difference], [*UB*: subtracting pointers into different arrays; a difference that does not fit `ptrdiff_t`], [chapters 27, 38],
)
...
#dtable(
  columns: 5,
  [*operator*], [*operands*], [*result*], [*grey zone*], [*in detail*],
  [`E1 << E2`], [both of *integer type*], [the type of the promoted *left* operand], [*UB*: `E2` negative or at least the width of the promoted `E1`. *UB*: `E1` signed and negative, or signed and positive with `E1 × 2^E2` not representable in the result type], [chapters 7, 28],
  [`E1 >> E2`], [both of integer type], [the same], [*UB*: `E2` negative or at least the width. *implementation-defined*: the result when `E1` is signed and negative], [chapters 7, 28],
)
...
#dtable(
  columns: 5,
  [*operator*], [*operands*], [*result*], [*grey zone*], [*in detail*],
  [`< <= > >=`], [both real types, or *two pointers to compatible object types*], [`0` or `1`, of type `int`], [*UB*: ordering two pointers that do not belong to the same array (or object)], [chapters 30, 37],
  [`== !=`], [both arithmetic, compatible pointers, one a `void*`, one a null pointer constant or `nullptr_t`, and so on], [`0` or `1`, `int`], [*unspecified*: whether a one-past-the-end pointer compares equal to a pointer to the object that follows], [chapters 30, 36],
)
...
`+0.0` and `-0.0` compare equal (chapter 49).
...
#dtable(
  columns: 5,
  [*operator*], [*operands*], [*result*], [*grey zone*], [*in detail*],
  [`&` `^` `|`], [both of integer type], [the common type of the usual arithmetic conversions], [they reach the sign bit, so use them on unsigned types], [chapter 28],
  [`&&`], [both scalar], [`0` or `1`, `int`], [*guaranteed*: if the left is 0 the right is not evaluated, and there is a sequence point between them], [chapter 30],
  [`||`], [both scalar], [`0` or `1`, `int`], [*guaranteed*: if the left is non-zero the right is not evaluated], [chapter 30],
)
...
#dtable(
  columns: 5,
  [*operator*], [*operands*], [*result*], [*grey zone*], [*in detail*],
  [`c ? a : b`], [`c` scalar. `a` and `b` both arithmetic, or compatible structs or unions, or both `void`, or compatible pointers, or one a null pointer constant], [*one common type* for both branches], [*guaranteed*: a sequence point after the condition; the branch not chosen is not evaluated], [chapter 33],
  ...
  [compound `op=`], [`E1 op= E2`], [as `E1 = E1 op E2`, except that *`E1` is evaluated once*], [the grey zones of the operation itself (overflow, zero divisor) still apply], [chapter 33],
  [`,` comma], [any two expressions], [the type and value of the right], [*guaranteed*: the left is evaluated and discarded, then a sequence point. The comma in an argument list is *not this operator*], [chapter 33],
)

== Evaluation order and sequence points

Precedence is a rule about *grouping*, not about the order in time (chapters 13
and 33). Order is guaranteed in exactly five places.
...
Only the operator-related entries, sorted by chapter 51's three words. The full
...
#dtable(
  ...
  [padding bytes], [the values of a struct's padding — the reason not to compare with `memcmp` (chapter 46)],
)
...
#dtable(
  ...
  [the comma in `f(a, b)`], [a separator of the call syntax — no sequence point], [chapter 33],
  [the comma in `int a, b;`], [a separator of declaration syntax], [chapter 23],
  [the comma in `{1, 2}`], [a separator in an initialiser list], [chapters 38, 45],
  [`(type){...}`], [a compound literal — not a cast but *syntax that makes an object*], [chapter 46],
  [the parentheses of `sizeof(int)`], [syntax wrapping a type name — not a call], [chapter 35],
  [`#` `##`], [preprocessor operators — they act in a different phase of translation], [chapter 56],
  [the dot in `{.x = 1}`], [designated-initialiser syntax, not member access], [chapter 45],
  [the star in `int *p;`], [declarator syntax, not indirection], [chapter 59],
)
...
approximation (chapter 49), handling failure (chapter 50), and what happens
when a contract is broken (chapter 51).
