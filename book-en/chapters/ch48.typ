...
#prereq(
  ...
  ([chapter 47, Errors and contracts], [what it means to break a contract]),
)
...
Why should such a thing be UB? Recall the *translation phases* seen in chapter 51 and
...
#dtable(
  ...
  [`int _Value;`, `int __x;`], [trespassing on the reserved name space (chapter 68)], [7.1.3],
...
#demo("examples-en/ch48/checked.c")
...
governed by the discipline learned in chapter 47 ("errors are values").

*Components* — using an API in which violating the contract is difficult to begin
with (chapter 39's proven is that layer). Chapter 17's metaphor — tools are nets,
...
The part of precision is over — how to handle approximation (chapter 46), how to
handle failure (chapter 47), and how to know the world outside the contract
(chapter 48). The three chapters share one theme: *C is a language that entrusts
