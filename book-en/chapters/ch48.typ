...
- *A success/failure boolean* plus receiving the result through a pointer
  (chapter 34's `&` idiom).
- *A special value* marking failure — `malloc`'s null (chapter 42), `fgets`'s
...
#demo("examples-en/ch48/errval.c")
...
#idx("const")chapter 43 as the mark "this function does not touch the original."
...
outright (chapter 39's string literals lived in that place).
...
as a `const` pointer instead of by value (chapter 43) is both safe and fast.
