...
#prereq(
  ([chapter 40, Strings], [strings]),
  ([chapter 9, Characters and text], [UTF-8 and bytes]),
)

#deepqa[
  Chapter 40 said that for a C string "up to the NUL" is the length, so to know the
...
#organizer[
  The answer to chapter 82's first bug — string functions that do not know the size
  ...
  truncate*. Chapter 9's story of encodings and chapter 40's story of boundaries
...
The rule set up in chapter 86 applied to strings as it is. `u8` means UTF-8 — that
...
```c
/* the borrowed string — the same shape as chapter 85's mem_view_t */
typedef struct {
...
#demo("examples-en/ch87/ops.c")

The first line shows chapter 86's rule again. `proven_u8str_borrow` has no allocator —
...
It is the exact opposite of `snprintf`'s choice seen in chapter 82. Why not truncate?
...
#qa[
  ...
  name (the same as chapter 85's `_unchecked`) is kept here too.
]
...
`_reserve`. As seen in chapter 86, `_reserve` pays especially on an arena — take it
...
kept — as seen in chapter 85, *it is valid only while the original is alive*.
...
Chapter 82 spoke of the danger of sentinel values, and what differs here is that *it
...
the place where chapter 82's third bug waits, and the place where this library's most
