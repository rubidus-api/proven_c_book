...
#prereq(
  ([chapter 55, Variadic functions], [variadic arguments and the format string]),
  ([chapter 58, The terrain of the standard library], [the printf contract]),
)

#deepqa[
  Chapter 55 said type information does not ride along into variadic arguments, and
  chapter 58 said that is why the format string alone settles "how the stack is to be
...
#organizer[
  The answer to chapter 81's third bug — the mismatch of format and argument. How the
  ...
  alternative to the `printf` and `scanf` taken apart in chapter 58.
]
...
The possibility of mismatch seen in chapter 58 is removed at the level of syntax.
...
They correspond to chapter 58's `printf` formats but differ in three ways. *The
...
`-0000042`, not `000000-42` — the same rule as `%08d` seen in chapter 58.
...
#demo("examples-en/ch87/fmt.c")

This example formats not to the screen but *into a string* — the `proven_println`
seen in chapter 82 is the edition connecting this machinery to standard output. Three
things can be pointed out.

*First, formatting too can fail.* `example.org:8080` cannot go into an 8-byte vessel,
so it was refused. Chapter 86's principle stands here too — rather than truncate, it
...
correspond to chapter 58's `%10s`, `%-10s` and `%.3f`, differing in that *there is no
type letter*.
...
Chapter 86's three kinds are here in formatting too. Organised in a table there is
...
`required` is the same information as `snprintf`'s return value seen in chapter 58. The
...
#qa[
  ...
  exact opposite of chapter 58's `printf`, which accepted anything.
]
...
#misconception[
  ...
  `%f` seen in chapter 58 is the more awkward task.
]
...
differs from expectation*. And as seen in chapter 58, `sscanf` tells only "how many
...
That `proven_scan_str` *returns a view without copying* matters — chapter 86's "text
handling without copying" holds in parsing too. In exchange that view is valid only
while the original input is alive (chapter 84).

#demo("examples-en/ch87/lines.c")
...
#qa[
  ...
  chapter 83 is not guaranteed here, so if it is really needed the cursor and the
...
#realcase[
  ...
  chapter 86's story of encodings — *do not read ambiguous input as something mended;
...
algorithms with "a guarantee even in the worst case" foretold in chapter 81.
