...
#demo("examples-en/ch87/fslife.c")
...
#demo("examples-en/ch87/fileio.c")

Several things stand out.

*The path is a view too.* `proven_u8str_view_from_cstr("...")` — wherever the string
came from, it is handled as a pointer and a length (chapter 84).
...
system requires. Chapter 83's rule is honestly kept here too — *if it can allocate, it
takes an allocator*.

*What is read becomes a view.* Bind the buffer and the number of bytes read and from
then on all of chapter 84's tools can be used. That is what the example used to divide
