...
#prereq(
  ([chapter 44, Dynamic memory], [dynamic allocation]),
  ([chapter 81, A program's map of memory], [a program's map of memory]),
)

#deepqa[
  Chapter 44 said one `malloc` is "a trip to the warehouse office", and
  chapter 81 showed where in the address space that warehouse lies. Then what
...
#organizer[
#idx("allocator")  We open the inside of the `malloc` chapter 44 passed over saying
...
This header explains two facts seen in chapter 44. *Even a request for 1 byte
...
Two things seen in chapter 81 attach here. When the heap is short it is enlarged
...
#realcase[
  ...
  The reason is, as seen in chapter 44, that `free` is not a return to the operating
...
chapter 44's first prescription — not allocating at all in the hot loop — is usually
the greater gain.
...
standard library implementation itself. It is the flesh of chapter 60's "the
...
threads and signals. Chapter 60's conclusion — "use only what is in the standard and
it ports" — pays here.
...
of function pointers learned in chapter 58 does exactly this work here.
...
#realcase[
  ...
  chapter 58's vtable. And there are three providers that make that value.
  ...
  argument. Second, the embedded norm seen in chapter 81 (no dynamic allocation) can
...
#qa[
  ...
  *things of assorted lifetimes*. The knack is fitting the tool to the purpose.
  ...
  And whichever is used, chapter 44's discipline stands — settle the ownership,
