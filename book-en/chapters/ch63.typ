...
#prereq(
  ([chapter 62, Streams in reality], [the state of a stream]),
  ([chapter 42, Safe input], [safe input]),
)

#deepqa[
  Chapter 42 said `gets` was deleted from the standard, and chapter 60 said its
  ...
  It took long because of the standard's character (chapter 61). Not breaking code
...
The hole the 1988 internet worm bored into was exactly this function (chapter 42).
...
#dtable(
  ...
  [`gets_s`], [C11 annex K (optional)], [implementations barely exist — treated in chapter 77],
)

`fgets` is the right answer, but it is not the end in itself. As seen in
chapter 62, if the buffer is too small it quietly reads only the front piece.

#demo("examples-en/ch63/reading.c")
...
Chapter 60 took the grammar apart, so here we gather only the places where
accidents happen.
...
variable that failed (we see it in the flesh in chapter 83).
...
contract. If the range must be checked, read with `strtol` (chapter 65).
...
#misconception[
  ...
  wrong file." We run this pattern for real in chapter 83.
]
...
*The format string vulnerability* — the one treated in chapter 60. The rule is
