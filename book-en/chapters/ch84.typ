...
#prereq(
  ([chapter 48, Undefined behaviour], [undefined behaviour]),
  ([chapter 47, Errors and contracts], [contracts]),
  ([chapter 82, C in practice], [the tools of practice]),
)
...
#deepqa[
  ...
  (chapter 47), avoid shortcuts outside the contract (chapter 48), check with tools
  (chapter 17). C23's new syntax appeared as tools that make that criterion *easier to
  keep*. The retrospect below is that list.
]

#organizer[
  The last chapter. Chapter 82 looked round the terrain of practice, so now we gather
...
#dtable(
  ...
  [`[[nodiscard]]`], [chapters 36 and 47], [the compiler blocks the mistake of forgetting to confirm failure],
  [`[[fallthrough]]`], [chapter 30], [the intent of switch fallthrough as documentation],
  [the `ckd_add` family], [chapter 48], [overflow as a value rather than UB],
  ...
  [the relaxation of `va_start`], [chapter 52], [removing the excess of variadic arguments],
)
...
#qa[
  ...
  to: data structures and algorithms (chapter 42's linked structures), concurrency
  ...
  directly the source of the projects named in chapter 82. Well-written C code like
