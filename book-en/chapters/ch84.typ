...
#prereq(
  ...
  ([chapter 77, How to ask about overflow], [arithmetic that does not overflow]),
)
...
#idx("view")  points at raw bytes, the *view* binding pointer and length into one,
  size calculation that does not wrap, and alignment. Chapter 81's sixth bug (the
...
#dtable(
  ...
  [`proven_mem_t`], [yes], [yes], [an allocator (chapter 85)],
...
overlap it is `proven_mem_move` — chapter 62's `memcpy`/`memmove` distinction as it
stands.
...
`uintptr_t` and then checks. It is the function chapter 85's arena uses when confirming
...
support range in chapter 90).

== Slicing — the operation used most in this part

#demo("examples-en/ch84/view.c")
...
was requested. It is blocking chapter 81's truncation problem from repeating here.
...
does not know what type is coming* uses `PROVEN_MAX_ALIGN`. It is also why chapter 85's
