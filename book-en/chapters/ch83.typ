...
#prereq(
  ([chapter 75, Errors are values], [returning failure as a value]),
  ([chapter 76, Foundations — byte, view, checked arithmetic], [borrowed slices and overflow checks]),
  ([chapter 77, Allocation — allocators, arenas, pools], [where memory comes from]),
  ([chapter 78, Strings], [refuse rather than truncate]),
)
...
#demo("examples-en/ch83/json_plain.c")
...
#demo("examples-en/ch83/json_proven.c")
...
slice pointing into the source buffer (chapter 76). With no container there is
...
(chapter 77). It does not know where the memory comes from and does not need to.
...
#demo("examples-en/ch83/json_nested.c")
...
#dtable(
  ...
  [Arena (chapter 77)], [Takes the memory of one parse in a lump and drops it in a lump], [Every node needs a matching `free`],
  [Pool (chapter 77)], [Recycles slots of exactly one `jnode`], [Same-size allocations fragment the heap],
  [Intrusive list (chapter 76)], [Hooks a child onto its parent — through a link inside the node], [A separate child array must be allocated and grown],
  [Dynamic array], [Stacks the open containers — the *explicit stack*], [You end up leaning on the call stack (that is, recursion)],
  [`view` (chapter 76)], [Borrows keys and strings from the source], [Every character needs a copy and a container],
  [Checked arithmetic (chapter 76)], [Watches overflow at every carry], [One forgotten `errno` and it wraps],
  [`proven_err_t` (chapter 75)], [Depth exceeded, no room, bad syntax — all as values], [Either death, or a silent trim],
)
...
#qa[
  ...
  That is why chapter 73's five bugs have been shipping for half a century. Not
