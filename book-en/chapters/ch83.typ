...
#prereq(
  ([chapter 50, Errors and contracts], [errors and contracts]),
  ([chapter 41, Strings], [the danger of strings]),
  ([chapter 60, The terrain of the standard library], [the traps of the standard library]),
)

#deepqa[
  Chapter 41 said the functions handling strings "do not know the size of the
  vessel", and chapter 50 said "a failure not confirmed becomes a thing that never
  ...
  institution that must protect existing code (chapter 60's reason for "thin and
...
The oldest and most exploited class. Exactly as seen in chapter 41.
...
function's return address (recall chapter 43's picture of the stack).
...
#demo("examples-en/ch83/truncate.c")
...
#demo("examples-en/ch83/unchecked.c")
...
#misconception[
  ...
  cause. Chapter 50's "fail early" returns here.
]

== Three — `printf` believes exactly what you tell it

Chapter 60 took the grammar of the format string apart. That grammar has one
...
cannot prevent it, because as seen in chapter 57 type information does not ride
...
#antipattern[
  ...
  *user input*, which becomes the format string vulnerability seen in chapter 60.
]
...
As learned in chapter 44, dynamically taken memory must be released by somebody
...
- A static buffer the next call will overwrite — it must neither be freed nor held
  for long (chapter 60's `strtok` was such).
...
chapter 44's three accidents — a leak (nobody frees), a double free (both free), and
...
#demo("examples-en/ch83/cmp_bad.c")
...
#realcase[
  ...
  *worst case* is itself a security problem, and so today's libraries take as their
  ...
  using a random seed — we see them in the flesh in chapter 90.
]
...
book has leaned on. We first met it in chapter 42 and its name has come up a few
