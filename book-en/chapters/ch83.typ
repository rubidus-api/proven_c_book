...
#prereq(
  ([chapter 52, Several files], [several files and linking]),
  ([chapter 82, The five bugs shipped for fifty years], [the five bugs]),
)

#deepqa[
  Chapter 52 made a multi-file program and learned about headers, object files and
#idx("the compilation process")  linking, and chapter 16 saw the four runners of
  ...
  *definition* — exactly chapter 52's picture. proven took the former road, and the
  next section is why.
]
...
#organizer[
  ...
  chapter 82 (format mismatch) already disappears in this first program. Then we follow
...
headers (chapter 52), `-lm` joins the mathematical functions. These two lines are
...
- So chapter 82's third bug — the mismatch of format and argument — *structurally*
...
and precision seen in chapter 59. `>` is right alignment, `<` left alignment, `.3` is
...
+ *Only a function that takes an allocator as an argument takes memory.* If
  ...
  embedded work and which are not divide before your eyes (chapter 86).
...
+ *Give a thing back with the allocator you made it with.* What was obtained with
  ...
  is not destroyed (chapters 85 and 86).
...
whether to give it the heap or an arena (chapter 86).
...
take `value` out — that order is the whole of chapter 84.
...
chapter 84's `goto` cleanup idiom.
...
#antipattern[
  ...
  your hand. ③ in particular is met again in chapter 87, and once more when an arena is
  reset.
]
...
#qa[
  ...
  (chapter 87), and several of this book's examples run so.

  There is a middle form too. Take the memory once in a large piece, lay an arena over
  it and hand out from there (chapter 86) — then `malloc` is never called once while the
...
To avoid typing the two lines above every time, use chapter 93's `make`. Supposing the
...
(chapter 52). *`platform/`* is the thin layer that calls the operating system, so when
going to bare metal only this line is removed (chapter 91). *`-lm`* joins the
mathematical functions that real-number formatting uses — take reals out of the
formatter (chapter 91's `PROVEN_FMT_NO_FLOAT`) and this is not needed either.
...
#platform[
  ...
  *Embedded* — leave out `platform/` and compile only `src/proven/*.c`. There being no
  ...
  laid over a static array is used instead (chapter 86). The detailed procedure is
  chapter 91.
]
