...
#prereq(
  ...
  ([chapter 81, The five bugs shipped for fifty years], [the unchecked return value]),
)
...
#deepqa[
  ...
  `-1` — that trap seen in chapter 81), take the result out as an *output parameter*
...
#organizer[
  We see the answer to chapter 81's second bug — unconfirmed failure. The way of
...
#dtable(
  ...
  [`proven_result_mem_mut_t`], [`proven_mem_mut_t`], [allocators (chapters 84 and 85)],
  [`proven_result_mem_view_t`], [`proven_mem_view_t`], [slicing (chapter 84)],
  [`proven_result_u8str_t`], [`proven_u8str_t`], [making a string (chapter 86)],
  [`proven_result_buf_t`], [`proven_buf_t`], [making a buffer],
  [`proven_result_cstr_t`], [`const char *`], [exporting as a C string (chapter 86)],
  [`proven_fmt_result_t`], [(amount written and amount needed)], [formatting (chapter 87)],
)
...
it carries two numbers as well (we look at it closely in chapter 87).

#demo("examples-en/ch83/errval.c")
...
exact opposite choice from `snprintf`'s quiet truncation seen in chapter 81.

== Throw it away and the compiler protests

Returning the error as a value is not enough by itself. As seen in chapter 81, a
...
#qa[
  But chapter 82's `proven_println` had no such mark. Why is screen output alone an
  exception?
][
  ...
  read, replaying chapter 81's second bug exactly. Where to attach the mark is itself
  a design judgement.
]
...
stack unwinds and destructors handle it, but C has no such device (chapter 72). So an
idiom is needed.

#demo("examples-en/ch83/cleanup.c")
...
+ *On failure everything gathers at one place* — `goto done`. That use chapter 72
...
#qa[
  ...
  As chapter 85 will show, an allocator is simply a value, so a shell that "fails from
...
usable in embedded work too (chapter 90).
...
functions with `_or_panic` in the name (chapter 85's arena allocation is
...
#realcase[
  ...
  *the error paths must be visible in the shape of the code*. Exceptions are
  ...
  chapter 81's table from the same direction.
]
