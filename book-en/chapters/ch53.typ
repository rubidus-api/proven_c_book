...
exist. That is why chapter 79 counts this among "the five bugs C has been
...
The `proven_println("{}", PROVEN_ARG(x))` used in chapter 85 is exactly this
...
#qa[
  ...
  stamps code out with macros (chapter 86's container macros) or passes `void *`
  with a size (chapter 79's `qsort`).
]
