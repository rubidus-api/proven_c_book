...
this book's choosing proven in chapter 38 is one instance of that choice.
...
The length modifier is not decoration but *a contract*. As chapter 50 showed, type
...
- *Functions that do not take a size* — the `strcpy`, `strcat` and `sprintf`
  families do not know the size of the destination vessel (chapter 38's `gets` was
...
- *Functions whose return value reports failure* — `malloc`, `fopen` and `fgets`
  give null on failure (chapter 45's discipline: do not use without checking).
- *The global state called `errno`* — many functions leave the reason for failure
  ...
  textbook case showing the inconvenience of global mutable state (chapter 39).
