...
#deepqa[
  ...
  chapter 52 said a variadic function loses type information. Then what does a
...
#demo("examples-en/ch53/funcptr.c")
...
table, not the code, and combined with chapter 51's X macro you can even generate
...
#demo("examples-en/ch53/vtable.c")
...
+ *The base struct is the first member.* The standard guarantees that "a struct's
  ...
  `struct circle *` and `struct shape *` can be safely gone between (chapter 42).
