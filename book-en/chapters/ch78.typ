...
#organizer[
  The answer to chapter 72's third bug — the mismatch of format and argument. How the
...
#demo("examples-en/ch78/fmt.c")

This example formats not to the screen but *into a string* — the `proven_println`
seen in chapter 73 is the edition connecting this machinery to standard output. Three
things can be pointed out.

*First, formatting too can fail.* `example.org:8080` cannot go into an 8-byte vessel,
so it was refused. Chapter 77's principle stands here too — rather than truncate, it
...
Chapter 77's three kinds are here in formatting too. Organised in a table there is
...
That `proven_scan_str` *returns a view without copying* matters — chapter 77's "text
handling without copying" holds in parsing too. In exchange that view is valid only
while the original input is alive (chapter 75).

#demo("examples-en/ch78/lines.c")
...
#qa[
  ...
  chapter 74 is not guaranteed here, so if it is really needed the cursor and the
...
#realcase[
  ...
  chapter 77's story of encodings — *do not read ambiguous input as something mended;
...
algorithms with "a guarantee even in the worst case" foretold in chapter 72.
