...
#prereq(
  ([chapter 75, What the new standards added, and the `*_s` controversy], [what the standards added]),
  ([chapter 29, Booleans and comparison], [the type of true and false]),
)

#deepqa[
  Chapter 75 passed over in a table only that C23 made `<stdbool.h>` effectively
...
#demo("examples-en/ch78/keywords.c")
...
standard does not break old code (chapter 61's `gets` story) was kept here too. But
...
#antipattern[
  ...
  if (a == 1) { … }          /* worse */
  ```
  As seen in chapter 64, the classification functions of `<ctype.h>` promise only
...
#qa[
  ...
  promoted to `int` (chapter 55's promotion rule), so `%d` is used — the example did
...
does not know the arguments' types and so reads the bits as they came (chapter 55).
...
In code that branches by type with `_Generic` (chapter 55), this ambiguity becomes
...
function pointer too (chapter 56). Two `nullptr`s, and a `nullptr` and any pointer,
...
the run-time `assert` (chapter 72) — this one is a tool asking "does this code hold
...
for avoiding the problem of sharing seen in chapter 76 *by not sharing*. `errno` is
in fact implemented this way (chapter 72).
...
#realcase[
  ...
  So the standard uses *names reserved so that users cannot use them* (chapter 52's
  table) — names
  beginning with one underscore and a capital letter. `_Bool`, `_Static_assert`,
  `_Alignas`, `_Atomic` (chapter 76) and `_Generic` are all products of this rule.
  ...
  old code remains even in the names — the same character as chapter 61's `gets`
...
on in an operating system and in an embedded chip respectively (chapter 79), and
what an allocator actually does in the heap region of that map (chapter 80). Those
