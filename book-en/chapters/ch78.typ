...
#prereq(
  ...
  ([chapter 50, Undefined behaviour], [overflow is outside the contract]),
)

#deepqa[
  Chapter 7 said unsigned overflow is defined wrap-round while signed overflow is
  outside the contract, and chapter 64 said `calloc` checks the product of the
...
same shell again. It is the same pattern as chapter 63's `strlcpy` story — reality
finds the answer first and the standard ratifies it belatedly.

== C23's answer — `ckd_add`, `ckd_sub`, `ckd_mul`

#demo("examples-en/ch78/ckdint.c")
...
#dtable(
  ...
  [numbers from input], [using it straight after `atoi`], [`strtol` (chapter 64) + a range check],
)
...
check at the input-parsing stage (chapter 64) and the check at the calculation
...
#misconception[
  ...
  `ckd_*`'s business. Third, *floating point* is not its subject (chapter 71).
...
real shape of the portability layer spoken of in chapter 55, and Part XII's library
does the same work.
