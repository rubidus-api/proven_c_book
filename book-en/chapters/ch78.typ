...
#prereq(
  ([chapter 42, Structs], [structs]),
  ([chapter 41, Dynamic memory], [a growing array]),
  ([chapter 74, The foundation], [views and bounds]),
)

#deepqa[
  Chapter 36 taught that a C array's size is settled at compile time, and chapter 41
  ...
  wrap-round seen in chapter 74 happens here. Second, *the state on failure* — when
...
#idx("hash map")  The tools that hold many — the growing array, the intrusive list,
  the ring buffer, the hash map. And as the answer to chapter 71's fifth bug
  ...
  Chapter 36's arrays and chapter 42's structs become practical components here.
]
...
especially loved in embedded work (chapter 80).
...
allocator per operation (chapter 76), while an array takes one at creation and keeps it
...
itself — and that growth happens through the allocator (exactly chapter 75's rule; the
...
#antipattern[
  ...
  *first = 7;                               /* writes at the old address — use after free */
  ...
  chapter 41 appears on a container. The rule is one — *after changing a container,
...
`proven_list_node_t link;` slot inside a `struct` made in chapter 42.
...
environment with no heap (chapter 80), and hanging the same object on two lists at
...
`link` member — the `offsetof` seen in chapter 42 used here in the flesh. This one line
...
and *an owned key* (the map copies and holds it). Chapter 75's owning-borrowing
...
that "absent" is not a failure but a normal answer (distinct from chapter 73's error
...
#demo("examples-en/ch78/wordcount.c")

This one example contains all of this chapter's tools — it counts with a map, gathers
into an array, sorts and prints. Chapter 76's views were used to cut the words, so
...
#qa[
  ...
  randomness — and in an environment with no OS (chapter 80) there is nowhere to draw
...
The two problems seen in chapter 71 — the unchecked comparator, and the algorithm
...
complexity attack seen in chapter 71 does not work. To carry the header's wording
...
by count, and *ties broken by the word*. It contrasts exactly with chapter 71's
...
- *hex and Base64* — two standards for moving bytes into text. The principle here is
  ...
  mended* but refused with `PROVEN_ERR_INVALID_ENCODING` (that norm from chapter 76).
