...
#prereq(
  ([chapter 77, What the new standards added, and the `*_s` controversy], [what the standards added]),
  ([chapter 30, Booleans and comparison], [the type of true and false]),
)

#deepqa[
  Chapter 77 passed over in a table only that C23 made `<stdbool.h>` effectively
...
#demo("examples-en/ch80/keywords.c")
...
standard does not break old code (chapter 63's `gets` story) was kept here too. But
...
#antipattern[
  ...
  if (a == 1) { … }          /* worse */
  ```
  As seen in chapter 66, the classification functions of `<ctype.h>` promise only
...
#qa[
  ...
  promoted to `int` (chapter 57's promotion rule), so `%d` is used — the example did
  ...
  when calculating a struct layout (chapter 45).
]

Two remaining traps of `bool`. First, using `bool` in a *bit-field* works fine even
with a width of 1, but the layout is implementation-defined (chapter 45). Second,
...
does not know the arguments' types and so reads the bits as they came (chapter 57).
...
In code that branches by type with `_Generic` (chapter 57), this ambiguity becomes
...
(chapter 53) where `NULL`'s meaning divided, and C23's adopting the same word
narrowed that gap.
...
function pointer too (chapter 58). Two `nullptr`s, and a `nullptr` and any pointer,
...
the run-time `assert` (chapter 74) — this one is a tool asking "does this code hold
...
for avoiding the problem of sharing seen in chapter 78 *by not sharing*. `errno` is
in fact implemented this way (chapter 74).
...
#realcase[
  ...
  So the standard uses *names reserved so that users cannot use them* (chapter 54's
  table) — names
  beginning with one underscore and a capital letter. `_Bool`, `_Static_assert`,
  `_Alignas`, `_Atomic` (chapter 78) and `_Generic` are all products of this rule.
  ...
  old code remains even in the names — the same character as chapter 63's `gets`
...
The two remaining chapters of this part go one layer down. The place chapter 44
passed over saying only "it is expensive" — what map a program's memory is laid out
on in an operating system and in an embedded chip respectively (chapter 81), and
what an allocator actually does in the heap region of that map (chapter 82). Those
