...
#prereq(
  ([chapter 65, What the new standards added, and the `*_s` controversy], [what the standards added]),
  ([chapter 29, Booleans and comparison], [the type of true and false]),
)

#deepqa[
  Chapter 65 passed over in a table only that C23 made `<stdbool.h>` effectively
...
#demo("examples-en/ch68/keywords.c")
...
standard does not break old code (chapter 58's `gets` story) was kept here too. But
...
#antipattern[
  ...
  if (a == 1) { … }          /* worse */
  ```
  As seen in chapter 61, the classification functions of `<ctype.h>` promise only
...
#qa[
  ...
  promoted to `int` (chapter 52's promotion rule), so `%d` is used — the example did
  ...
  when calculating a struct layout (chapter 42).
]

Two remaining traps of `bool`. First, using `bool` in a *bit-field* works fine even
with a width of 1, but the layout is implementation-defined (chapter 42). Second,
...
does not know the arguments' types and so reads the bits as they came (chapter 52).
...
In code that branches by type with `_Generic` (chapter 52), this ambiguity becomes
...
(chapter 50) where `NULL`'s meaning divided, and C23's adopting the same word
narrowed that gap.
...
function pointer too (chapter 53). Two `nullptr`s, and a `nullptr` and any pointer,
...
the run-time `assert` (chapter 64) — this one is a tool asking "does this code hold
...
for avoiding the problem of sharing seen in chapter 66 *by not sharing*. `errno` is
in fact implemented this way (chapter 64).
...
#realcase[
  ...
  `_Alignas`, `_Atomic` (chapter 66) and `_Generic` are all products of this rule.
  ...
  old code remains even in the names — the same character as chapter 58's `gets`
...
The two remaining chapters of this part go one layer down. The place chapter 41
passed over saying only "it is expensive" — what map a program's memory is laid out
on in an operating system and in an embedded chip respectively (chapter 69), and
what an allocator actually does in the heap region of that map (chapter 70). Those
