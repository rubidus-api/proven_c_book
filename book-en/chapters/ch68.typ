...
This header explains two facts seen in chapter 40. *Even a request for 1 byte
...
same ledger it becomes slow through locking (chapter 64), so each strand keeps a
...
Two things seen in chapter 67 attach here. When the heap is short it is enlarged
...
#realcase[
  ...
  The reason is, as seen in chapter 40, that `free` is not a return to the operating
...
chapter 40's first prescription — not allocating at all in the hot loop — is usually
the greater gain.
...
standard library implementation itself. It is the flesh of chapter 53's "the
...
threads and signals. Chapter 53's conclusion — "use only what is in the standard and
it ports" — pays here.
...
of function pointers learned in chapter 51 does exactly this work here.
...
#realcase[
  ...
  chapter 51's vtable. And there are three providers that make that value.
  ...
  argument. Second, the embedded norm seen in chapter 67 (no dynamic allocation) can
...
#qa[
  ...
  *things of assorted lifetimes*. The knack is fitting the tool to the purpose.
  ...
  And whichever is used, chapter 40's discipline stands — settle the ownership,
