...
#prereq(
  ([chapter 51, Several files], [a project of several files]),
  ([chapter 17, Setting up a development environment], [setting up the tools]),
)

#deepqa[
  Chapter 51 said that for a multi-file program the practice is to make each file
...
chapter 16's relay and chapter 51's division save time in practice.
...
#dtable(
  ...
  [`massif`, `heaptrack`], [heap usage over time], [leaks and over-allocation (chapter 42)],
...
expensive allocations (chapter 42) are crowded, all become visible as they are.
...
#platform[
  ...
  - *Measuring stack usage* — the stack painting seen in chapter 79, and static
...
#dtable(
  ...
  [Dynamic allocation (`malloc`/`free`)], [Fragmentation and unpredictable timing (chapters 42, 80); arenas and pools instead],
  [Recursion], [You cannot know how much stack it will take (chapter 79)],
  ...
  [Variadic functions], [No type checking (chapter 55)],
  [Type punning through a union], [Code that leans on representation (chapter 45)],
  [Overuse of `#define` macros], [The preprocessor knows no types (chapter 54)],
  [Parts of the standard library], [Things like `gets`, `atoi`, `strcpy` (chapters 61, 62)],
)
...
#dtable(
  ...
  [`_Generic` (chapter 55)], [exceptions and RAII],
  [order-independent designated initialisers], [namespaces],
  [compound literals (chapter 44)], [`auto` type deduction, lambdas],
)
...
#demo("examples-en/ch92/ccompat.c")
...
function name it plants in the object file (recall chapter 51's linking), while a C
...
the standard library (chapter 58's freestanding) and the code generated is predictable.
...
and 27), not using dynamic allocation at all (chapter 42), handling communication
protocols conscious of alignment and representation (chapters 6 and 45).
...
- *No namespaces* → the prefix convention, file-level `static` and symbol
  visibility (chapters 52 and 53 — strictly, the standard defines four name spaces;
  what you cannot do is *make a new one*).
- *No generics* → code generation with macros (chapter 54's `##`, the X macro), or
...
- *Resource cleanup is not automatic* → state the ownership convention in names and
  documentation (chapter 42), and gather one function's cleanup points into one place
...
- *The complexity of large projects* → divide the translation units small and reveal the
  contracts through headers (chapter 51), and automate the tests and the build (this
...
#qa[
  ...
  statistics, a simple configuration-file parser (chapter 87's scanner is used as it
  stands), implementing one data structure yourself (chapter 43's linked structures),
