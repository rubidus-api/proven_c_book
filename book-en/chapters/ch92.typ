...
#prereq(
  ([chapter 78, Operations that do not split], [operations that do not split]),
  ([chapter 81, A program's map of memory], [when there is no OS]),
)
...
header and it is handled only by pointer (the opaque type of chapter 59 in the flesh).
...
threads use one arena together, the simple action seen in chapter 87 of "cutting from
...
(as in chapter 87's counterexample) even if the address happens to match. The library's
name came from here.

== When there is no OS — freestanding

This is the constraint that most shaped this library. It is the demand to run in the
very environment chapter 60 gave as the reason the standard library is thin — a place
...
- Allocation is done with an arena over a static buffer (chapter 87). There is no
  `malloc`.
- Strings are handled with `_borrow` over stack and static buffers (chapter 88).
...
argument*, *lifetime is per arena*, *`main` does not return* (chapter 52), and *the
...
- *Work that one standard function finishes* — exactly as this book said in
  chapter 60. The right attitude is neither "do not use it" nor "use it
...
#qa[
  ...
  Open chapter 83's table again and the right-hand column is entirely such items. What
...
#recap[
  ...
  #dtable(
  columns: 3,
    [*chapter 83's problem*], [*the answer*], [*chapter*],
    [strings that do not know the size], [views (ptr+size), refusing rather than truncating], [chapters 86 and 88],
    [unconfirmed failure], [errors as values, `[[nodiscard]]`], [chapter 85],
    [format mismatch], [`{}` and `PROVEN_ARG` (`_Generic`)], [chapter 89],
    [unclear ownership], [the allocator parameter, owning versus borrowing], [chapter 87],
    [unchecked callbacks], [documented contracts, introsort, keyed hashes], [chapter 90],
    [the hidden type of bytes], [`proven_byte_t`], [chapter 86],
    [environments with no OS], [separating the platform layer, static arenas], [chapter 92],
)
]
