...
#prereq(
  ...
  ([chapter 33, The meaning of a function], [side effects and evaluation order]),
  ([chapter 29, Implicit conversions], [when values of different types meet]),
)

#deepqa[
  Chapter 33 said that calls with side effects should be given their own
...
#idx("assignment")  The last chapter of this part is the operator used most often
  ...
  chapter 48's gathering of the operators and chapter 51's undefined behaviour.
]
...
#demo("examples-en/ch34/assign.c")
...
still outside the contract, and so is `x <<= 40` (chapter 28).
...
#qa[
  ...
  moves by elements (chapter 38). And a habit inherited from C++ — "use `++x` when
...
knowledge "on this compiler it comes out like this" (chapter 51's principle).
...
#antipattern("gathering side effects into one expression")[
  ...
  compiler may generate anything at all and owes you no diagnostic (chapter 51).
...
Chapter 29's conversion rules apply here quietly, and quietly is the danger.
...
(truncation toward zero, chapter 28).
...
#dtable(
  ...
  [`float f = 0.1;`], [narrowed from double to single], [fine; precision is lost (chapter 49)],
)
...
Part VI is over — we opened the map of types and forged values (chapters 26–29),
governed flow (chapters 30–32), and gained the meaning of functions and of
assignment (chapters 33–34). The next part is this book's second mountain, *memory*: back to
