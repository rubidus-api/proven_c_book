...
#prereq(
  ([chapter 53, Several files], [a project of several files]),
  ([chapter 17, Setting up a development environment], [setting up the tools]),
)

#deepqa[
  Chapter 53 said that for a multi-file program the practice is to make each file
...
chapter 16's relay and chapter 53's division save time in practice.
...
#dtable(
  ...
  [`massif`, `heaptrack`], [heap usage over time], [leaks and over-allocation (chapter 44)],
...
expensive allocations (chapter 44) are crowded, all become visible as they are.
...
grey areas and chapter 51's undefined behaviour are handled in practice is written
...
#dtable(
  ...
  [Dynamic allocation (`malloc`/`free`)], [Fragmentation and unpredictable timing (chapters 44, 82); arenas and pools instead],
  [Recursion], [You cannot know how much stack it will take (chapter 81)],
  [Code that relies on undefined behaviour], [The whole of chapter 51],
  ...
  [Variadic functions], [No type checking (chapter 57)],
  [Type punning through a union], [Code that leans on representation (chapter 47)],
  [Overuse of `#define` macros], [The preprocessor knows no types (chapter 56)],
  [Parts of the standard library], [Things like `gets`, `atoi`, `strcpy` (chapters 63, 64)],
)
...
#dtable(
  ...
  [`_Generic` (chapter 57)], [exceptions and RAII],
  [order-independent designated initialisers], [namespaces],
  [compound literals (chapter 46)], [`auto` type deduction, lambdas],
)
...
#demo("examples-en/ch94/ccompat.c")
...
function name it plants in the object file (recall chapter 53's linking), while a C
...
the standard library (chapter 60's freestanding) and the code generated is predictable.
...
and 28), not using dynamic allocation at all (chapter 44), handling communication
protocols conscious of alignment and representation (chapters 6 and 47).
...
- *No namespaces* → the prefix convention, file-level `static` and symbol
  visibility (chapters 54 and 55 — strictly, the standard defines four name spaces;
  what you cannot do is *make a new one*).
- *No generics* → code generation with macros (chapter 56's `##`, the X macro), or
...
- *Resource cleanup is not automatic* → state the ownership convention in names and
  documentation (chapter 44), and gather one function's cleanup points into one place
...
- *The complexity of large projects* → divide the translation units small and reveal the
  contracts through headers (chapter 53), and automate the tests and the build (this
...
#qa[
  ...
  statistics, a simple configuration-file parser (chapter 89's scanner is used as it
  stands), implementing one data structure yourself (chapter 45's linked structures),
