...
#prereq(
  ...
  ([chapter 75, The foundation], [views and bounds]),
)
...
#deepqa[
  ...
  wrap-round seen in chapter 75 happens here. Second, *the state on failure* — when
...
#idx("hash map")  The tools that hold many — the growing array, the intrusive list,
  the ring buffer, the hash map. And as the answer to chapter 72's fifth bug
...
especially loved in embedded work (chapter 81).
...
allocator per operation (chapter 77), while an array takes one at creation and keeps it
...
itself — and that growth happens through the allocator (exactly chapter 76's rule; the
...
environment with no heap (chapter 81), and hanging the same object on two lists at
...
and *an owned key* (the map copies and holds it). Chapter 76's owning-borrowing
...
that "absent" is not a failure but a normal answer (distinct from chapter 74's error
...
#demo("examples-en/ch79/wordcount.c")

This one example contains all of this chapter's tools — it counts with a map, gathers
into an array, sorts and prints. Chapter 77's views were used to cut the words, so
...
#qa[
  ...
  randomness — and in an environment with no OS (chapter 81) there is nowhere to draw
...
The two problems seen in chapter 72 — the unchecked comparator, and the algorithm
...
complexity attack seen in chapter 72 does not work. To carry the header's wording
...
by count, and *ties broken by the word*. It contrasts exactly with chapter 72's
...
- *hex and Base64* — two standards for moving bytes into text. The principle here is
  ...
  mended* but refused with `PROVEN_ERR_INVALID_ENCODING` (that norm from chapter 77).
