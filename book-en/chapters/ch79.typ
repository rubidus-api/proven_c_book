...
chapter 16's relay and chapter 48's division save time in practice.
...
#dtable(
  ...
  [`massif`, `heaptrack`], [heap usage over time], [leaks and over-allocation (chapter 40)],
...
expensive allocations (chapter 40) are crowded, all become visible as they are.
...
#platform[
  ...
  - *Measuring stack usage* — the stack painting seen in chapter 67, and static
...
#dtable(
  ...
  [variable-length arrays (chapter 36)], [templates and the standard library upon them],
  [flexible array members], [references (`&`) and operator overloading],
  [`_Generic` (chapter 50)], [exceptions and RAII],
  [order-independent designated initialisers], [namespaces],
  [compound literals (chapter 42)], [`auto` type deduction, lambdas],
)
...
function name it plants in the object file (recall chapter 48's linking), while a C
...
the standard library (chapter 53's freestanding) and the code generated is predictable.
...
and 27), not using dynamic allocation at all (chapter 40), handling communication
protocols conscious of alignment and representation (chapters 6 and 43).
...
- *No namespaces* → the prefix convention (chapter 38's `proven_`) and file-level
  `static` (chapter 48's internal linkage).
- *No generics* → code generation with macros (chapter 49's `##`, the X macro), or
...
- *Resource cleanup is not automatic* → state the ownership convention in names and
  documentation (chapter 40), and gather one function's cleanup points into one place
...
- *The complexity of large projects* → divide the translation units small and reveal the
  contracts through headers (chapter 48), and automate the tests and the build (this
...
#qa[
  ...
  statistics, a simple configuration-file parser (chapter 75's scanner is used as it
  stands), implementing one data structure yourself (chapter 41's linked structures),
