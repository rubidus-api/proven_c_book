...
#prereq(
  ([chapter 76, What the new standards added, and the `*_s` controversy], [what the standards added]),
  ([chapter 30, Booleans and comparison], [the type of true and false]),
)

#deepqa[
  Chapter 76 passed over in a table only that C23 made `<stdbool.h>` effectively
...
#demo("examples-en/ch79/keywords.c")
...
standard does not break old code (chapter 62's `gets` story) was kept here too. But
...
#antipattern[
  ...
  if (a == 1) { … }          /* worse */
  ```
  As seen in chapter 65, the classification functions of `<ctype.h>` promise only
...
#qa[
  ...
  promoted to `int` (chapter 56's promotion rule), so `%d` is used — the example did
  ...
  when calculating a struct layout (chapter 44).
]

Two remaining traps of `bool`. First, using `bool` in a *bit-field* works fine even
with a width of 1, but the layout is implementation-defined (chapter 44). Second,
an array of `bool` uses one byte per element — it is not compressed into bits like
C++'s `vector<bool>`. To compress into bits, write the masks by hand (chapter 28)
...
does not know the arguments' types and so reads the bits as they came (chapter 56).
...
In code that branches by type with `_Generic` (chapter 56), this ambiguity becomes
...
(chapter 52) where `NULL`'s meaning divided, and C23's adopting the same word
narrowed that gap.
...
function pointer too (chapter 57). Two `nullptr`s, and a `nullptr` and any pointer,
...
#misconception[
  ...
  human's part (chapter 36), and reducing that burden is the part not of the
...
the run-time `assert` (chapter 73) — this one is a tool asking "does this code hold
...
for avoiding the problem of sharing seen in chapter 77 *by not sharing*. `errno` is
in fact implemented this way (chapter 73).
...
#realcase[
  ...
  So the standard uses *names reserved so that users cannot use them* (chapter 53's
  table) — names
  beginning with one underscore and a capital letter. `_Bool`, `_Static_assert`,
  `_Alignas`, `_Atomic` (chapter 77) and `_Generic` are all products of this rule.
  ...
  old code remains even in the names — the same character as chapter 62's `gets`
...
The two remaining chapters of this part go one layer down. The place chapter 43
passed over saying only "it is expensive" — what map a program's memory is laid out
on in an operating system and in an embedded chip respectively (chapter 80), and
what an allocator actually does in the heap region of that map (chapter 81). Those
