...
#prereq(
  ...
  ([chapter 51, Undefined behaviour], [overflow is outside the contract]),
)

#deepqa[
  Chapter 7 said unsigned overflow is defined wrap-round while signed overflow is
  outside the contract, and chapter 65 said `calloc` checks the product of the
...
same shell again. It is the same pattern as chapter 64's `strlcpy` story — reality
finds the answer first and the standard ratifies it belatedly.

== C23's answer — `ckd_add`, `ckd_sub`, `ckd_mul`

#demo("examples-en/ch79/ckdint.c")
...
+ *The result type is settled by the first argument (the pointer).* It means the
  arguments' promotion rules (chapter 28) do not sway the result, so there is no
...
#dtable(
  ...
  [numbers from input], [using it straight after `atoi`], [`strtol` (chapter 65) + a range check],
)
...
check at the input-parsing stage (chapter 65) and the check at the calculation
...
#misconception[
  ...
  `ckd_*`'s business. Third, *floating point* is not its subject (chapter 72).
...
real shape of the portability layer spoken of in chapter 56, and Part XII's library
does the same work.
