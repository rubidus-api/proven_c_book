...
#prereq(
  ([chapter 45, Structs], [the layout of a struct]),
...
#deepqa[
  ...
  *A type whose several members share the same memory.* If a struct lays members
  side by side (chapter 45), a union lays them *overlapping* — its size fits the
...
#demo("examples-en/ch47/endian.c")
...
chapter 46 — the purpose here is to confirm with our own eyes that the gap
*really exists*.
...
#demo("examples-en/ch47/punning.c")
...
#demo("examples-en/ch47/bitfield.c")
...
same reason and the same prescription as chapter 46's serialisation. Bit fields
...
side (chapter 45) and the union that lays them over one another (chapter 47). And
...
comes down into C's floating types (chapter 49), the perspective of the contract
whose seed was planted in chapter 33 grows into error handling (chapter 50), and
we meet head on the world "outside the contract" that this book has foreshadowed
throughout — undefined behaviour (chapter 51).
