...
#dtable(
  ...
  [`<stdarg.h>`], [variadic arguments], [chapter 50],
...
#dtable(
  ...
  [`<assert.h>`], [C89], [`assert` — the diagnosis that catches contract violations], [chapter 62],
  [`<complex.h>`], [C99], [complex arithmetic], [chapter 60],
  [`<ctype.h>`], [C89], [character classification and conversion], [chapter 59],
  [`<errno.h>`], [C89], [the error-number global], [chapter 62],
  [`<fenv.h>`], [C99], [the floating-point environment (rounding, exceptions)], [chapter 60],
  [`<float.h>`], [C89], [the limits of real types], [chapters 26, 60],
  [`<inttypes.h>`], [C99], [formats and conversions for fixed-width integers], [appendix B, chapter 63],
  [`<iso646.h>`], [C95], [alternative spellings of operators], [chapter 63],
  [`<limits.h>`], [C89], [the limits of integer types], [chapter 26],
  [`<locale.h>`], [C89], [locale settings], [chapter 59],
  [`<math.h>`], [C89], [mathematical functions], [chapter 60],
  [`<setjmp.h>`], [C89], [non-local jumps], [chapter 62],
  [`<signal.h>`], [C89], [signal handling], [chapter 62],
  [`<stdalign.h>`], [C11], [specifying and querying alignment], [chapter 63],
  [`<stdarg.h>`], [C89], [variadic arguments], [chapter 50],
  [`<stdatomic.h>`], [C11], [atomic operations], [chapter 64],
  [`<stdbit.h>`], [C23], [bit manipulation (counting, rotating and so on)], [chapter 63],
  [`<stdbool.h>`], [C99], [`bool`, `true`, `false`], [chapter 63],
  [`<stdckdint.h>`], [C23], [arithmetic that checks for overflow], [chapters 46, 65],
  [`<stddef.h>`], [C89], [`size_t`, `ptrdiff_t`, `NULL`, `offsetof`], [chapter 63],
  [`<stdint.h>`], [C99], [fixed-width integer types], [chapters 26, 63],
  [`<stdio.h>`], [C89], [stream input and output, files], [chapters 48, 56],
  [`<stdlib.h>`], [C89], [conversion, random numbers, allocation, sorting, program termination], [chapter 58],
  [`<stdnoreturn.h>`], [C11], [`noreturn`], [chapter 63],
  [`<string.h>`], [C89], [strings and memory blocks], [chapter 57],
  [`<tgmath.h>`], [C99], [type-generic mathematical functions], [chapter 60],
  [`<threads.h>`], [C11], [threads, mutexes, condition variables], [chapter 63],
  [`<time.h>`], [C89], [time and the calendar], [chapter 61],
  [`<uchar.h>`], [C11], [UTF-16 and UTF-32 character types], [chapter 59],
  [`<wchar.h>`], [C95], [wide-character input, output and strings], [chapter 59],
  [`<wctype.h>`], [C95], [wide-character classification], [chapter 59],
)
...
#misconception[
  ...
  the 1989 standard and was deleted only in 2011 (chapter 56). `strncpy`, contrary
  to its name, is not a safe copying function (chapter 57), and `atoi` has no way
  at all to report failure (chapter 58). Some functions of *the same name even
  behave differently according to the locale* (chapter 59). Using the standard
