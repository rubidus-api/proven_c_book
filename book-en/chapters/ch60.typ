...
#prereq(
  ([chapter 59, The terrain of the standard library], [the character of the standard library]),
)

#deepqa[
  Chapter 59 said the standard library is "thin and old", and that the standard
...
#organizer[
  ...
  those regions. It is the chapter that redraws chapter 59's terrain map at a
  larger scale.
]
...
#dtable(
  ...
  [`<stdarg.h>`], [variadic arguments], [chapter 56],
...
#dtable(
  ...
  [`<assert.h>`], [C89], [`assert` — the diagnosis that catches contract violations], [chapter 73],
  [`<complex.h>`], [C99], [complex arithmetic], [chapter 71],
  [`<ctype.h>`], [C89], [character classification and conversion], [chapter 65],
  [`<errno.h>`], [C89], [the error-number global], [chapter 73],
  [`<fenv.h>`], [C99], [the floating-point environment (rounding, exceptions)], [chapter 71],
  [`<float.h>`], [C89], [the limits of real types], [chapters 27, 71],
  [`<inttypes.h>`], [C99], [formats and conversions for fixed-width integers], [appendix B, chapter 76],
  [`<iso646.h>`], [C95], [alternative spellings of operators], [chapter 76],
  [`<limits.h>`], [C89], [the limits of integer types], [chapter 27],
  [`<locale.h>`], [C89], [locale settings], [chapter 65],
  [`<math.h>`], [C89], [mathematical functions], [chapter 71],
  [`<setjmp.h>`], [C89], [non-local jumps], [chapter 73],
  [`<signal.h>`], [C89], [signal handling], [chapter 73],
  [`<stdalign.h>`], [C11], [specifying and querying alignment], [chapter 76],
  [`<stdarg.h>`], [C89], [variadic arguments], [chapter 56],
  [`<stdatomic.h>`], [C11], [atomic operations], [chapter 77],
  [`<stdbit.h>`], [C23], [bit manipulation (counting, rotating and so on)], [chapter 76],
  [`<stdbool.h>`], [C99], [`bool`, `true`, `false`], [chapter 76],
  [`<stdckdint.h>`], [C23], [arithmetic that checks for overflow], [chapters 50, 78],
  [`<stddef.h>`], [C89], [`size_t`, `ptrdiff_t`, `NULL`, `offsetof`], [chapter 76],
  [`<stdint.h>`], [C99], [fixed-width integer types], [chapters 27, 76],
  [`<stdio.h>`], [C89], [stream input and output, files], [chapters 52, 62],
  [`<stdlib.h>`], [C89], [conversion, random numbers, allocation, sorting, program termination], [chapter 64],
  [`<stdnoreturn.h>`], [C11], [`noreturn`], [chapter 76],
  [`<string.h>`], [C89], [strings and memory blocks], [chapter 63],
  [`<tgmath.h>`], [C99], [type-generic mathematical functions], [chapter 71],
  [`<threads.h>`], [C11], [threads, mutexes, condition variables], [chapter 76],
  [`<time.h>`], [C89], [time and the calendar], [chapter 72],
  [`<uchar.h>`], [C11], [UTF-16 and UTF-32 character types], [chapter 65],
  [`<wchar.h>`], [C95], [wide-character input, output and strings], [chapter 65],
  [`<wctype.h>`], [C95], [wide-character classification], [chapter 65],
)
...
#misconception[
  ...
  the 1989 standard and was deleted only in 2011 (chapter 62). `strncpy`, contrary
  to its name, is not a safe copying function (chapter 63), and `atoi` has no way
  at all to report failure (chapter 64). Some functions of *the same name even
  behave differently according to the locale* (chapter 65). Using the standard
