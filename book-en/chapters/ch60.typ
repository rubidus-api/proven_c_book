...
#prereq(
  ([chapter 42, Safe input], [the traps of the standard functions]),
...
#deepqa[
  ...
  joins it up, and chapter 42 said `gets` was *removed* from the standard. Who
...
chapter 42's five disciplines are about what to demand of that choice.
...
#demo("examples-en/ch60/fmtspec.c")
...
The length modifier is not decoration but *a contract*. As chapter 57 showed, type
...
#misconception[
  ...
  argument promotion (chapter 29), so both are `%f` — the standard permits `%lf`
...
knowing the destination's size, the same danger as chapter 42's `gets`.

#demo("examples-en/ch60/scanspec.c")
...
- *Functions that do not take a size* — the `strcpy`, `strcat` and `sprintf`
  families do not know the size of the destination vessel (chapter 42's `gets` was
...
- *Functions whose return value reports failure* — `malloc`, `fopen` and `fgets`
  give null on failure (chapter 50's discipline: do not use without checking).
- *The global state called `errno`* — many functions leave the reason for failure
  ...
  textbook case showing the inconvenience of global mutable state (chapter 43).
