...
#prereq(
  ([chapter 64, Character classification], [that the judgement depends on the locale]),
  ([chapter 50, The three faces of `main`], [environment variables]),
)

#deepqa[
  Chapter 64 said the answer `isalpha` gives depends on "the current locale". But
...
#demo("examples-en/ch65/locale_probe.c")
...
#dtable(
  ...
  [`LC_CTYPE`], [Classification, case, *multibyte conversion*], [`isalpha` family (chapter 64), `mbrtowc` family (chapter 67)],
...
comes from that line (chapter 66).
...
#qa[
  ...
  *numbers a machine will read and write are pinned* so the locale cannot move
  them. These two lines prevent most of the data corruption seen in chapter 66.
...
#realcase[
  ...
  in Korea, and it is also why the three layers of chapter 69 — bytes, code
