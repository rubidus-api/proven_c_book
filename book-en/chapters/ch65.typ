...
#prereq(
  ...
  ([chapter 27, Integers], [the signedness of `char`]),
)
...
The trouble is that `char` is signed on many implementations (chapter 27). As the
...
argument too.* What `fgetc` returns must be passable straight in (chapter 61), and
...
the first trap. Chapter 61's rule, "always receive what `fgetc` returns in an
`int`", comes back here.
...
characters* and use `<wctype.h>`'s `iswalpha` (chapters 68 and 69), or *work on
the byte string as it is* and do only the judgements you need yourself
(chapter 70). This book recommends the latter, and chapter 70 says why.
