...
#prereq(
  ([chapter 56, The whole map of the standard library], [the whole map of the standard library]),
  ([chapter 58, The traps of reading and writing], [bounds and truncation]),
)

#deepqa[
  Chapter 56 said the speed at which headers grow is the same as the language's
  speed of change, and chapter 58 said `gets`'s funeral took twenty years. Then
...
#dtable(
  ...
  [`<fenv.h>`], [the floating-point environment], [chapter 62],
  [`<tgmath.h>`], [type-generic mathematics], [chapter 62],
)
...
#qa[
  ...
  *API designs that carry the length along*. The last is the direction this book
  has pushed, and chapters 74 and 76 are its implementation.
  ...
  chapter 71 arranges the five bugs.
]
...
#dtable(
  ...
  [`<uchar.h>`], [`char16_t`, `char32_t`], [chapter 61],
)
...
#demo("examples-en/ch65/newheaders.c")

`<stdckdint.h>` is this edition's practical winner. It reports the wrap-round of
the size calculations seen in chapter 59 *as a value* — `ckd_add`, `ckd_sub` and
...
The direction resembles what we organised in chapter 56 as "what is needed". Yet
