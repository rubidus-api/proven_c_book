...
#prereq(
  ([chapter 60, The terrain of the standard library], [the terrain of the standard library]),
  ([chapter 44, Dynamic memory], [allocating and giving back]),
)

#deepqa[
  Chapter 25 read input with `fgets` and parsed it with `sscanf`, and chapter 60
...
#demo("examples-en/ch65/convert.c")
...
#antipattern[
  ...
  ```
  *Setting `errno` to 0 just before the call* is the convention (chapter 74).
...
decimal point as `,` rather than `.` (chapter 66) — a point always to be remembered
...
#qa[
  ...
  90's `proven_array_sort` pins the type with a macro, and why chapter 83 counts
  "the unchecked callback" among the five bugs.
]

== Dynamic allocation — four functions and their contracts

We organise what chapter 44 taught, function by function.
...
product wrap round as seen in chapter 64, but for `calloc(k, n)` the standard
...
#antipattern[
  ...
  buf = tmp;
  ```
  It is why chapter 63's line-reading example used this idiom.
]
...
`abort` do not — the "output vanishing" accident seen in chapter 63 happens here.
...
The functions chapter 61 foretold. Written out exactly, the contract is this.
...
- The comparator must be a *total order* (chapter 83). If it is inconsistent the
...
- Worst-case performance is settled by the implementation. The standard guarantees
  nothing — the reason chapter 61's complexity attack was possible.
