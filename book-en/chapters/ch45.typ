...
#prereq(
  ...
  ([Chapter 42, Structures], [member access]),
)
...
- *Grey zone* — in the three words of chapter 48. *UB* is undefined
...
#dtable(
  ...
  [`f(...)` call], [a function, or a pointer to one], [the function's return type; not an lvalue], [*unspecified*: the order in which arguments are evaluated. *UB*: arguments that disagree with the prototype], [chapters 21, 24, 52],
  [`s.m` member], [a struct or union value and a member name], [the member's type; an lvalue if the left side is one], [*UB*: reading a union member other than the one last written (the common initial sequence is an exception)], [chapters 42, 44],
  [`p->m` member], [a pointer to a struct or union, and a member name], [the member's type, an lvalue], [*UB*: a null or otherwise invalid pointer], [chapter 42],
...
#dtable(
  ...
  [`*p` indirection], [a pointer type], [an lvalue of the pointed-at type], [*UB*: null, an object whose lifetime has ended, a misaligned address, or one outside its provenance], [chapters 34, 36, 40],
...
#dtable(
  ...
  [`/` divide], [arithmetic types], [the same], [*UB*: a zero divisor, `INT_MIN / -1`], [chapters 27, 46],
...
`+0.0` and `-0.0` compare equal (chapter 46).
...
Only the operator-related entries, sorted by chapter 48's three words. The full
...
#dtable(
  ...
  [padding bytes], [the values of a struct's padding — the reason not to compare with `memcmp` (chapter 43)],
)
...
#dtable(
  ...
  [the comma in `{1, 2}`], [a separator in an initialiser list], [chapters 37, 42],
  [`(type){...}`], [a compound literal — not a cast but *syntax that makes an object*], [chapter 43],
  [the parentheses of `sizeof(int)`], [syntax wrapping a type name — not a call], [chapter 34],
  [`#` `##`], [preprocessor operators — they act in a different phase of translation], [chapter 51],
  [the dot in `{.x = 1}`], [designated-initialiser syntax, not member access], [chapter 42],
  [the star in `int *p;`], [declarator syntax, not indirection], [chapter 54],
)
...
approximation (chapter 46), handling failure (chapter 47), and what happens
when a contract is broken (chapter 48).
