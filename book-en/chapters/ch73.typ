...
#prereq(
  ...
  ([chapter 72, A program's map of memory], [a program's map of memory]),
)

#deepqa[
  Chapter 42 said one `malloc` is "a trip to the warehouse office", and
  chapter 72 showed where in the address space that warehouse lies. Then what
...
same ledger it becomes slow through locking (chapter 69), so each strand keeps a
...
Two things seen in chapter 72 attach here. When the heap is short it is enlarged
...
#realcase[
  ...
  argument. Second, the embedded norm seen in chapter 72 (no dynamic allocation) can
