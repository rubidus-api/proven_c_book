...
#prereq(
  ([chapter 49, Errors and contracts], [reporting an error as a value]),
  ([chapter 50, Undefined behaviour], [after a contract is broken]),
)
...
#organizer[
  ...
  stack. Why the discipline set up in chapter 49, "errors are values", matters so
...
#demo("examples-en/ch73/errno_demo.c")
...
#dtable(
  ...
  [`strerror_s`], [the same intent], [annex K (chapter 76)],
)
...
#qa[
  ...
  `assert`. In chapter 49's vocabulary of contracts, `assert` confirms
...
The macro learned in chapter 49. Pinning down the rules again:
...
in servers, shells and terminals, all in chapter 74.
...
software such as libjpeg and Lua, and today's alternatives, all in chapter 75.
