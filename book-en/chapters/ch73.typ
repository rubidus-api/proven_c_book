...
#prereq(
  ([chapter 72, Diagnostics and control], [`errno` and the handler's restrictions]),
  ([chapter 50, The three faces of `main`], [a program's start and end]),
  ([chapter 3, Programs and processes], [the process as a container]),
)

#deepqa[
  Chapter 72 said a signal handler may not call `printf` or `malloc`, and that
...
#qa[
  ...
  operating system* (chapter 59). `sigaction` stands on operating-system notions —
...
Why `signal`'s declaration is rough was read by procedure in chapter 57 — *"a
...
if there is no handler or the handler returns (chapter 64).

#demo("examples-en/ch73/sig_basic.c")
...
#dtable(
  ...
  [Handling lock-free atomic objects], [`<stdatomic.h>`, when lock-free (chapter 76)],
  [Calling `abort`], [—],
  [Calling `_Exit`], [—],
  [Calling `quick_exit`], [—],
...
handler (chapter 76). In a program with several threads that is the more accurate
...
(chapter 74). Here the trouble comes without calling `malloc` again — because you
*jump out still holding the lock*. From then on the main flow's first `malloc`
hangs. Half the reason chapter 74 calls jumping from a handler dangerous is this.
...
#demo("examples-en/ch73/sig_context.c")
...
#demo("examples-en/ch73/sig_flag.c")
...
#demo("examples-en/ch73/sig_action.c")
...
Ignored, `write` returns the failure *as a value* (`errno == EPIPE`). Chapter 72's
