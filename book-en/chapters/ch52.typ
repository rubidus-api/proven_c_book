...
chapter 41's rule that array parameters decay into pointers). The names are free
...
environment variables is `getenv` (chapter 62).
...
#antipattern[
  ...
  implementation* (chapter 60) the name and form of the starting function are the
...
#qa[
  ...
  *Write it yourself* — for a short program this is enough. Scan `argv`,
  ...
  slot. To convert to a number use `strtol`, not `atoi` (chapter 62).
  ...
  *A library* — as the scale grows (subcommands, generated help, merging with a
  ...
  Whichever road, keep one rule: *arguments are input from outside.* Chapter 43's
...
#platform[
  ...
  *Windows* — `CreateProcess` passes *one string* (chapter 3). That is, the dividing
  ...
  rules) — the same grain as chapter 65's `system` counterexample.
]
...
after `main` ended. Flushing open streams (chapter 62's buffers) is included in
...
#qa[
  ...
  *the C program's point of view*. In reality, startup code (something like `crt0`)
  ...
  calling `main` (chapter 81). `main` is not "the first code that runs" but "the
