...
That the word `static` is used with different meanings in chapter 42 (static
...
dig a new name space (chapter 53), so every external name meets in one yard; attach
`static` to helper
...
collisions are the subject of chapter 54.
