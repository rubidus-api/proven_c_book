...
#prereq(
  ([chapter 61, The whole map of the standard library], [the whole map of the standard library]),
  ([chapter 63, The traps of reading and writing], [bounds and truncation]),
)

#deepqa[
  Chapter 61 said the speed at which headers grow is the same as the language's
  speed of change, and chapter 63 said `gets`'s funeral took twenty years. Then
...
#dtable(
  ...
  [`<fenv.h>`], [the floating-point environment], [chapter 72],
  [`<tgmath.h>`], [type-generic mathematics], [chapter 72],
)
...
#qa[
  ...
  *API designs that carry the length along*. The last is the direction this book
  has pushed, and chapters 86 and 88 are its implementation.
  ...
  chapter 83 arranges the five bugs.
]
...
#dtable(
  ...
  [`<uchar.h>`], [`char16_t`, `char32_t`], [chapter 66],
)
...
#demo("examples-en/ch77/newheaders.c")

`<stdckdint.h>` is this edition's practical winner. It reports the wrap-round of
the size calculations seen in chapter 64 *as a value* — `ckd_add`, `ckd_sub` and
...
The direction resembles what we organised in chapter 61 as "what is needed". Yet
