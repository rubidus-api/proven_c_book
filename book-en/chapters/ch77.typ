...
#deepqa[
  ...
  outside the contract, and chapter 63 said `calloc` checks the product of the
...
same shell again. It is the same pattern as chapter 62's `strlcpy` story — reality
finds the answer first and the standard ratifies it belatedly.

== C23's answer — `ckd_add`, `ckd_sub`, `ckd_mul`

#demo("examples-en/ch77/ckdint.c")
...
#dtable(
  ...
  [numbers from input], [using it straight after `atoi`], [`strtol` (chapter 63) + a range check],
)
...
check at the input-parsing stage (chapter 63) and the check at the calculation
...
#misconception[
  ...
  `ckd_*`'s business. Third, *floating point* is not its subject (chapter 70).
...
real shape of the portability layer spoken of in chapter 54, and Part XII's library
does the same work.
