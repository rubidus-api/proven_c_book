...
#prereq(
  ([chapter 41, Lifetime and storage duration], [storage duration]),
  ([chapter 2, The regions of memory], [the regions of memory]),
)

#deepqa[
  Chapter 41 said "in the C standard there is neither the word stack nor any
...
#idx("stack size")  We redraw at a larger scale the map of memory sketched in
  chapter 41. What is absent from the C standard and present only in the operating
...
Writing out again the order chapter 41's example showed, but this time separating
...
That *`.bss` has only its size in the file* is the heart of this table (chapter 41
...
(chapter 41). This limit is not a *reservation* but an upper bound, so in reality
...
chapter 42's "`free` mostly does not return to the OS".

*④ Addresses change on every run — ASLR.* Run chapter 41's example twice and the
...
array, deep recursion, and calls passing a large struct by value (chapter 44). The
...
world of the *freestanding implementation* seen in chapter 56. Here most of the
...
in chapter 54 — where the address spaces of code and data are entirely different —
becomes flesh here.
...
function pointers about (chapter 54) leaves the compiler unable to know the graph
...
#misconception[
  ...
  It can be used, but here chapter 42's reasons weigh far more. First, *there is
...
#qa[
  ...
  The fact seen in chapter 44 that "passing a struct by value makes the stack jump
