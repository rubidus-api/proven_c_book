...
#organizer[
  The answer to chapter 72's first bug — string functions that do not know the size
...
The rule set up in chapter 76 applied to strings as it is. `u8` means UTF-8 — that
...
```c
/* the borrowed string — the same shape as chapter 75's mem_view_t */
typedef struct {
...
#demo("examples-en/ch77/ops.c")

The first line shows chapter 76's rule again. `proven_u8str_borrow` has no allocator —
...
It is the exact opposite of `snprintf`'s choice seen in chapter 72. Why not truncate?
...
#qa[
  ...
  name (the same as chapter 75's `_unchecked`) is kept here too.
]
...
`_reserve`. As seen in chapter 76, `_reserve` pays especially on an arena — take it
...
kept — as seen in chapter 75, *it is valid only while the original is alive*.
...
Chapter 72 spoke of the danger of sentinel values, and what differs here is that *it
...
the place where chapter 72's third bug waits, and the place where this library's most
