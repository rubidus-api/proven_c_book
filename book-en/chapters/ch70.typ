...
#prereq(
  ([chapter 69, Wide characters ①], [`wchar_t`'s contract and conversion]),
  ([chapter 62, Streams in practice], [streams and buffers]),
)

#deepqa[
  Chapter 69 said the standard fixes neither the size nor the encoding of
...
Chapter 69's demonstration gave this machine's answer — four bytes, and
...
#demo("examples-en/ch70/surrogate.c")
...
in chapter 71's validation). That is why trying to hold U+D800 as UTF-16 printed
...
#dtable(
  ...
  [It is tied to the locale], [Conversion fails unless `LC_CTYPE` is UTF-8 (chapter 69)],
...
mostly useful (chapter 71). So the Linux camp took the road of "UTF-8 inside
too."
...
#demo("examples-en/ch70/wide_io.c")
