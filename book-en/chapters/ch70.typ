...
comparator's total order and the result collapses (chapter 63).
...
mean to look at `errno`, set it to 0 just before the call (chapter 72).
...
#qa[
  ...
  and check immediately after (chapter 72). To inspect the value itself use
...
alone — the representative case of the `_Generic` seen in chapter 55 being used in
