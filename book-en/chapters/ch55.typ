...
#demo("examples-en/ch55/parens.c")
...
and left only a trailing comma (that comma being legal thanks to chapter 44's
initialiser syntax).
...
textbook case of *unspecified behaviour*, one of the grey zones learned in chapter 50 —
...
The `__VA_OPT__` C23 brought in (chapter 79) puts something in "only when the variadic
...
#demo("examples-en/ch55/xmacro.c")
...
Moreover it uses the *designated initialiser* `[code] = #name` (chapter 44) so
...
#qa[
  ...
  (chapter 52). The macro line continuations (`\\`) disappear and the list reads
