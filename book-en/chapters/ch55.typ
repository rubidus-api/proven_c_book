...
#prereq(
  ([chapter 39, Safe input, and the appearance of proven], [the traps of the standard functions]),
...
#deepqa[
  ...
  joins it up, and chapter 39 said `gets` was *removed* from the standard. Who
...
this book's choosing proven in chapter 39 is one instance of that choice.
...
#demo("examples-en/ch55/fmtspec.c")
...
The length modifier is not decoration but *a contract*. As chapter 52 showed, type
...
knowing the destination's size, the same danger as chapter 39's `gets`.

#demo("examples-en/ch55/scanspec.c")
...
- *Functions that do not take a size* — the `strcpy`, `strcat` and `sprintf`
  families do not know the size of the destination vessel (chapter 39's `gets` was
...
- *Functions whose return value reports failure* — `malloc`, `fopen` and `fgets`
  give null on failure (chapter 47's discipline: do not use without checking).
- *The global state called `errno`* — many functions leave the reason for failure
  ...
  textbook case showing the inconvenience of global mutable state (chapter 40).
