...
(chapter 58), and why writing `va_arg(ap, float)` is a contract violation, taking
...
exist. That is why chapter 81 counts this among "the five bugs C has been
...
#demo("examples-en/ch55/generic.c")
...
The `proven_println("{}", PROVEN_ARG(x))` used in chapter 87 is exactly this
...
#qa[
  ...
  stamps code out with macros (chapter 88's container macros) or passes `void *`
  with a size (chapter 81's `qsort`).
]
