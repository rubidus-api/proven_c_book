...
#prereq(
  ([chapter 53, Many files — splitting and linking], [external and internal linkage]),
  ([chapter 54, The world of names], [four name spaces and three axes]),
)

#deepqa[
  Chapter 54 said C has only four name spaces and that you cannot make a new
...
file-scope declaration and it has internal linkage (chapter 53): outside that
...
+ *Put it on types and macros too.* A prefix that covers only functions is half a
  fence --- chapter 54 showed that type names and enumeration constants live in the
  same yard.
...
#realcase[
  ...
  goes elsewhere. That is what chapter 54's table of reserved names is for, and
...
#qa[
  ...
  *the ABI and existing code.* Bringing in name spaces means the yard must be carved
  ...
  call" precisely because its symbol names are simple (chapter 94).
...
#dtable(
  ...
  [Prefix types, macros and enumeration constants too], [all three share one yard (ch. 54)],
  ...
  [Turn on `-Wshadow`], [shadowing is not caught by the default warnings (ch. 54)],
)

We know how to govern names. But chapter 54 went past the remark that "macros ignore
