...
#dtable(
  ...
  [allocated], [`malloc` ~ `free`], [chapter 41],
)
...
same object*. It is the groundwork of chapter 48 (several files).
...
- *`typedef`* — syntactically it goes in this slot but does something entirely
  ...
  chapter 52).
- *`thread_local`* — makes an object that is separate per thread (chapter 63). It
...
#qa[
  ...
  definition in one source file comes from here (chapter 48).
]
...
#dtable(
  ...
  [heap], [what `malloc` gave (chapter 41)], [until freed],
...
#platform[
  ...
  *The C standard has neither the word "stack" nor any promise about its size.* It
  ...
  work, are treated in chapter 67.
]
...
Second, why chapter 38's boundary-violation attack is so dangerous — overflow an
...
#misconception[
  ...
  a container the caller provided (chapter 33's `&` idiom). Chapter 17's ASan is
...
#qa[
  ...
  the subject of chapter 48 (linkage). And the practical advice is an old one:
...
settled at run time and which stays alive as long as you wish — chapter 40's
dynamic memory.
