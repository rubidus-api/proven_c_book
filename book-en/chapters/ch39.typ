...
#prereq(
  ...
  ([chapter 38, Strings], [a string only marks its end]),
)

#deepqa[
  Chapter 38's gets was expelled for not taking the container's size, and
...
#organizer[
  Chapter 25 foreshadowed "why safe input is difficult", and chapters 34 and 38
...
Combine chapter 37's boundary rule with chapter 38's NUL termination and the most
...
epidemic running unbroken from the Morris worm (1988, chapter 38) to today's
...
flesh of the "errors are values" discipline treated formally in chapter 47.
...
#qa[
  ...
  that cross file boundaries live in one yard — chapter 50). If libraries use the
