...
#prereq(
  ([chapter 51, Undefined behaviour], [undefined behaviour]),
  ([chapter 50, Errors and contracts], [contracts]),
  ([chapter 94, C in practice], [the tools of practice]),
)
...
#deepqa[
  ...
  the boundary and the size together (chapter 38), confirm failure as a value
  (chapter 50), avoid shortcuts outside the contract (chapter 51), check with tools
  (chapter 17). C23's new syntax appeared as tools that make that criterion *easier to
  keep*. The retrospect below is that list.
]

#organizer[
  The last chapter. Chapter 94 looked round the terrain of practice, so now we gather
...
#dtable(
  ...
  [the `bool`, `true`, `false` keywords], [chapter 30], [judgement as a value without a header],
  [`nullptr`], [chapter 36], [removing `NULL`'s ambiguity],
  [two's complement fixed], [chapter 7], [the end of worrying about the portability of sign representation],
  [`[[nodiscard]]`], [chapters 37 and 50], [the compiler blocks the mistake of forgetting to confirm failure],
  [`[[fallthrough]]`], [chapter 31], [the intent of switch fallthrough as documentation],
  [the `ckd_add` family], [chapter 51], [overflow as a value rather than UB],
  [`alignof`], [chapter 37], [querying alignment requirements at the level of the language],
  [the removal of trigraphs], [chapter 9], [tidying up a scar of the character-code era],
  [the relaxation of `va_start`], [chapter 57], [removing the excess of variadic arguments],
)
...
`<stdint.h>`. Keep sizes and indexes consistently in the `size_t` family (chapter 29's
...
#qa[
  ...
  to: data structures and algorithms (chapter 45's linked structures), concurrency
  ...
  directly the source of the projects named in chapter 94. Well-written C code like
