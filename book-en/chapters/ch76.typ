...
#prereq(
  ...
  ([chapter 69, How to ask about overflow], [arithmetic that does not overflow]),
)
...
#idx("view")  points at raw bytes, the *view* binding pointer and length into one,
  size calculation that does not wrap, and alignment. Chapter 73's sixth bug (the
...
#dtable(
  ...
  [`proven_mem_t`], [yes], [yes], [an allocator (chapter 77)],
...
`uintptr_t` and then checks. It is the function chapter 77's arena uses when confirming
...
support range in chapter 82).

== Slicing — the operation used most in this part

#demo("examples-en/ch76/view.c")
...
was requested. It is blocking chapter 73's truncation problem from repeating here.
...
does not know what type is coming* uses `PROVEN_MAX_ALIGN`. It is also why chapter 77's
