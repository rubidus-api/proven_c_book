...
#deepqa[
  Chapter 41 said that calling a function stacks a frame, and that `return`
...
#demo("examples-en/ch76/jmp_basic.c")
...
#demo("examples-en/ch76/jmp_volatile.c")
...
#misconception[
  ...
  This is `volatile`'s second legitimate use (the first was chapter 75's
  ...
  `<stdatomic.h>` (chapter 78). Miss this distinction and `volatile` becomes a
...
#demo("examples-en/ch76/jmp_leak.c")
...
#antipattern("Code that reverts to an old address and frees it twice")[
  ...
  ```
  Chapter 44's double free, reproduced exactly. The allocator's ledger is wrecked,
...
#demo("examples-en/ch76/jmp_error.c")
...
#realcase("libjpeg's error manager — how it came to look like this")[
  ...
  take chapter 50's "failure as a value", but within the constraints of the 1990s
...
The difference is whether the *signal mask* (chapter 75) is saved along with the
...
#misconception[
  ...
  *Second, and more fundamentally, you do not know where it was cut.* A signal
  arrives in the middle of `malloc` or `printf` too (chapter 75). Jump out from
...
#dtable(
  ...
  [Carry a deep failure upward], [Return it as a value (chapter 50). Tedious for the middle layers, but safer],
