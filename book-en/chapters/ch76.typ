...
#prereq(
  ...
  ([chapter 71, Inside the allocator], [inside the allocator]),
)
...
#organizer[
  The answer to chapter 72's fourth bug — unclear ownership. The *allocator* that
...
— because the allocator wrote it into the block's header (chapter 71). But an arena has
...
+ *`align` must be a power of two.* And *a block must be reallocated and freed with the
  ...
  or below the default alignment differently from stricter ones (chapter 75) — handing
...
#qa[
  ...
  Of course. Chapter 74's example already did — a testing allocator that "fails from the
  ...
  *Special resources* — obtaining from shared memory or a DMA-capable region, places
  ...
  allocator inside, do their own work and pass it on. Chapter 74's example is the model.
]
...
#qa[
  ...
  with no heap* — such is embedded work (chapter 81), and such is inside a kernel.
...
The `_or_panic` family exists for the reason given in chapter 74 — where the memory
...
chapter 77's `proven_u8str_reserve` in advance pays especially on an arena (an
intervening allocation breaks the property).

#demo("examples-en/ch76/arena.c")
...
#misconception[
  ...
  still be alive after the reset must not be put in the arena. Chapter 75's "a view
...
#dtable(
  ...
  [data of assorted lifetimes], [the heap], [general-purpose. individual release possible], [cost and fragmentation (chapter 71)],
  ...
  [testing the out-of-memory path], [a failing shell (chapter 74)], [it runs the path that never otherwise runs], [for testing only],
)
...
Now we return to chapter 72's fourth bug. The problem of `char *` meaning four things
