...
#prereq(
  ([chapter 38, Strings], [a string is an array that only marks its end]),
  ([chapter 37, Arrays], [handling memory in bulk]),
)

#deepqa[
  Chapter 38 said a C string is "up to the NUL", so its length must be counted
  every time, and chapter 56 said functions that do not take a size are the first
...
#organizer[
  ...
  dangers of strings learned in chapter 38 take concrete shape here function by
...
#demo("examples-en/ch59/strncpy.c")
...
But as seen in chapter 56, the compiler catches *only what it can see*. If the
...
#demo("examples-en/ch59/overlap.c")
...
*Comparing structs with `memcmp`* — because of the padding seen in chapter 42.
