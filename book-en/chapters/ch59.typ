...
#prereq(
  ([chapter 58, The terrain of the standard library], [the character of the standard library]),
)

#deepqa[
  Chapter 58 said the standard library is "thin and old", and that the standard
...
#organizer[
  ...
  those regions. It is the chapter that redraws chapter 58's terrain map at a
  larger scale.
]
...
#dtable(
  ...
  [`<stdarg.h>`], [variadic arguments], [chapter 55],
...
#dtable(
  ...
  [`<assert.h>`], [C89], [`assert` — the diagnosis that catches contract violations], [chapter 72],
  [`<complex.h>`], [C99], [complex arithmetic], [chapter 70],
  [`<ctype.h>`], [C89], [character classification and conversion], [chapter 64],
  [`<errno.h>`], [C89], [the error-number global], [chapter 72],
  [`<fenv.h>`], [C99], [the floating-point environment (rounding, exceptions)], [chapter 70],
  [`<float.h>`], [C89], [the limits of real types], [chapters 26, 70],
  [`<inttypes.h>`], [C99], [formats and conversions for fixed-width integers], [appendix B, chapter 75],
  [`<iso646.h>`], [C95], [alternative spellings of operators], [chapter 75],
  [`<limits.h>`], [C89], [the limits of integer types], [chapter 26],
  [`<locale.h>`], [C89], [locale settings], [chapter 64],
  [`<math.h>`], [C89], [mathematical functions], [chapter 70],
  [`<setjmp.h>`], [C89], [non-local jumps], [chapter 72],
  [`<signal.h>`], [C89], [signal handling], [chapter 72],
  [`<stdalign.h>`], [C11], [specifying and querying alignment], [chapter 75],
  [`<stdarg.h>`], [C89], [variadic arguments], [chapter 55],
  [`<stdatomic.h>`], [C11], [atomic operations], [chapter 76],
  [`<stdbit.h>`], [C23], [bit manipulation (counting, rotating and so on)], [chapter 75],
  [`<stdbool.h>`], [C99], [`bool`, `true`, `false`], [chapter 75],
  [`<stdckdint.h>`], [C23], [arithmetic that checks for overflow], [chapters 49, 77],
  [`<stddef.h>`], [C89], [`size_t`, `ptrdiff_t`, `NULL`, `offsetof`], [chapter 75],
  [`<stdint.h>`], [C99], [fixed-width integer types], [chapters 26, 75],
  [`<stdio.h>`], [C89], [stream input and output, files], [chapters 51, 61],
  [`<stdlib.h>`], [C89], [conversion, random numbers, allocation, sorting, program termination], [chapter 63],
  [`<stdnoreturn.h>`], [C11], [`noreturn`], [chapter 75],
  [`<string.h>`], [C89], [strings and memory blocks], [chapter 62],
  [`<tgmath.h>`], [C99], [type-generic mathematical functions], [chapter 70],
  [`<threads.h>`], [C11], [threads, mutexes, condition variables], [chapter 75],
  [`<time.h>`], [C89], [time and the calendar], [chapter 71],
  [`<uchar.h>`], [C11], [UTF-16 and UTF-32 character types], [chapter 64],
  [`<wchar.h>`], [C95], [wide-character input, output and strings], [chapter 64],
  [`<wctype.h>`], [C95], [wide-character classification], [chapter 64],
)
...
#misconception[
  ...
  the 1989 standard and was deleted only in 2011 (chapter 61). `strncpy`, contrary
  to its name, is not a safe copying function (chapter 62), and `atoi` has no way
  at all to report failure (chapter 63). Some functions of *the same name even
  behave differently according to the locale* (chapter 64). Using the standard
