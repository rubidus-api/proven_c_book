...
#prereq(
  ([chapter 41, Safe input], [the traps of the standard functions]),
...
#deepqa[
  ...
  joins it up, and chapter 41 said `gets` was *removed* from the standard. Who
...
chapter 41's five disciplines are about what to demand of that choice.
...
#demo("examples-en/ch59/fmtspec.c")
...
The length modifier is not decoration but *a contract*. As chapter 56 showed, type
...
knowing the destination's size, the same danger as chapter 41's `gets`.

#demo("examples-en/ch59/scanspec.c")
...
- *Functions that do not take a size* — the `strcpy`, `strcat` and `sprintf`
  families do not know the size of the destination vessel (chapter 41's `gets` was
...
- *Functions whose return value reports failure* — `malloc`, `fopen` and `fgets`
  give null on failure (chapter 49's discipline: do not use without checking).
- *The global state called `errno`* — many functions leave the reason for failure
  ...
  textbook case showing the inconvenience of global mutable state (chapter 42).
