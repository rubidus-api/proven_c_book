...
int when a double really arrived — and it is outside the contract (chapter 51).
...
(chapter 60), and why writing `va_arg(ap, float)` is a contract violation, taking
...
exist. That is why chapter 83 counts this among "the five bugs C has been
...
#demo("examples-en/ch57/generic.c")
...
The `proven_println("{}", PROVEN_ARG(x))` used in chapter 89 is exactly this
...
value itself (exactly chapter 47's tagged union).
...
#qa[
  ...
  stamps code out with macros (chapter 90's container macros) or passes `void *`
  with a size (chapter 83's `qsort`).
]
...
#misconception[
  ...
  kinds vary, take an explicit list of values with chapter 47's tagged union. The
