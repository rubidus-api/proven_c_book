...
#demo("examples-en/ch41/alloc_cost.c")
...
the arena and pool approaches we see later were born (chapter 70).
...
+ *With several threads, locks appear.* The warehouse ledger is a shared resource,
  so contention between threads slows it (chapter 66's story of races). That is
...
and cut them up. The last is the arena, and chapter 70 and Part XII are that
story.
...
#misconception[
  ...
  that a large lump cannot be formed: *fragmentation*. Chapter 70 faces it head
  on.
]
...
#demo("examples-en/ch41/flat2d.c")
...
and there is one place to fix. When writing the macro, keep chapter 51's rules —
...
example checks with `ckd_mul` (chapters 48 and 67) first and does not even attempt
...
#realcase[
  ...
  use the address of a slot that was given back. It is chapter 40's dangling
...
#qa[
  ...
  boundaries together, like chapter 39's proven).
]
...
chapters 35–36, contiguous memory and strings in chapters 37–38, a safe component
in chapter 39, and lifetime and the warehouse in chapters 40–41. We have gone
...
at the end of Part XI (chapters 69 and 70).
