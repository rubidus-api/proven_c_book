...
#deepqa[
  ...
  chapter 39 used the phrase "the return ledger of a function call". Are a name
...
#dtable(
  ...
  [allocated], [`malloc` ~ `free`], [chapter 42],
)
...
same object*. It is the groundwork of chapter 50 (several files).
...
- *`typedef`* — syntactically it goes in this slot but does something entirely
  ...
  chapter 54).
- *`thread_local`* — makes an object that is separate per thread (chapter 65). It
...
#qa[
  ...
  definition in one source file comes from here (chapter 50).
]
...
#demo("examples-en/ch40/regions.c")
...
#dtable(
  ...
  [heap], [what `malloc` gave (chapter 42)], [until freed],
...
#platform[
  ...
  *The C standard has neither the word "stack" nor any promise about its size.* It
  ...
  work, are treated in chapter 69.
]
...
Second, why chapter 39's boundary-violation attack is so dangerous — overflow an
...
#qa[
  ...
  the subject of chapter 50 (linkage). And the practical advice is an old one:
...
settled at run time and which stays alive as long as you wish — chapter 41's
dynamic memory.
