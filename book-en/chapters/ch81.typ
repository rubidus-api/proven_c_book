...
#deepqa[
  ...
  the boundary and the size together (chapter 36), confirm failure as a value
...
#dtable(
  ...
  [`nullptr`], [chapter 34], [removing `NULL`'s ambiguity],
  [two's complement fixed], [chapter 7], [the end of worrying about the portability of sign representation],
  [`[[nodiscard]]`], [chapters 35 and 45], [the compiler blocks the mistake of forgetting to confirm failure],
  [`[[fallthrough]]`], [chapter 30], [the intent of switch fallthrough as documentation],
  [the `ckd_add` family], [chapter 46], [overflow as a value rather than UB],
  [`alignof`], [chapter 35], [querying alignment requirements at the level of the language],
  [the removal of trigraphs], [chapter 9], [tidying up a scar of the character-code era],
  [the relaxation of `va_start`], [chapter 50], [removing the excess of variadic arguments],
)
...
#qa[
  ...
  to: data structures and algorithms (chapter 41's linked structures), concurrency
  ...
  directly the source of the projects named in chapter 79. Well-written C code like
