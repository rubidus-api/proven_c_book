...
#prereq(
  ([chapter 67, Operations that do not split], [operations that do not split]),
  ([chapter 70, A program's map of memory], [when there is no OS]),
)
...
threads use one arena together, the simple action seen in chapter 76 of "cutting from
...
(as in chapter 76's counterexample) even if the address happens to match. The library's
name came from here.
...
- Allocation is done with an arena over a static buffer (chapter 76). There is no
  `malloc`.
- Strings are handled with `_borrow` over stack and static buffers (chapter 77).
...
#qa[
  ...
  Open chapter 72's table again and the right-hand column is entirely such items. What
...
#recap[
  ...
  #dtable(
  columns: 3,
    [*chapter 72's problem*], [*the answer*], [*chapter*],
    [strings that do not know the size], [views (ptr+size), refusing rather than truncating], [chapters 75 and 77],
    [unconfirmed failure], [errors as values, `[[nodiscard]]`], [chapter 74],
    [format mismatch], [`{}` and `PROVEN_ARG` (`_Generic`)], [chapter 78],
    [unclear ownership], [the allocator parameter, owning versus borrowing], [chapter 76],
    [unchecked callbacks], [documented contracts, introsort, keyed hashes], [chapter 79],
    [the hidden type of bytes], [`proven_byte_t`], [chapter 75],
    [environments with no OS], [separating the platform layer, static arenas], [chapter 81],
)
]
