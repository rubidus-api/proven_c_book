...
#prereq(
  ([chapter 65, Diagnostics and control], [`errno` and the handler's restrictions]),
  ([chapter 50, The three faces of `main`], [a program's start and end]),
  ([chapter 3, Programs and processes], [the process as a container]),
)

#deepqa[
  Chapter 65 said a signal handler may not call `printf` or `malloc`, and that
...
#demo("examples-en/ch66/sig_basic.c")
...
#demo("examples-en/ch66/sig_flag.c")
...
#demo("examples-en/ch66/sig_action.c")
...
Ignored, `write` returns the failure *as a value* (`errno == EPIPE`). Chapter 65's
