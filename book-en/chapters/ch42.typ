...
the arena and pool approaches we see later were born (chapter 78).
...
+ *With several threads, locks appear.* The warehouse ledger is a shared resource,
  so contention between threads slows it (chapter 74's story of races). That is
...
and cut them up. The last is the arena, and chapter 78 and Part XII are that
story.
...
#misconception[
  ...
  that a large lump cannot be formed: *fragmentation*. Chapter 78 faces it head
  on.
]
...
and there is one place to fix. When writing the macro, keep chapter 52's rules —
...
example checks with `ckd_mul` (chapters 49 and 75) first and does not even attempt
...
at the end of Part XI (chapters 77 and 78).
