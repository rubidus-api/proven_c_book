...
chapter 39's rule that array parameters decay into pointers). The names are free
...
#qa[
  ...
  *A library* — as the scale grows (subcommands, generated help, merging with a
  ...
  Whichever road, keep one rule: *arguments are input from outside.* Chapter 41's
...
#qa[
  ...
  *the C program's point of view*. In reality, startup code (something like `crt0`)
  ...
  calling `main` (chapter 70). `main` is not "the first code that runs" but "the
