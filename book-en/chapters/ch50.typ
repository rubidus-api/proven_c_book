...
int when a double really arrived — and it is outside the contract (chapter 46).
...
widths are out of step. That is why C23's `nullptr` (chapter 34) or an explicit
cast (`(char *)0`) is used.
...
exist. That is why chapter 69 counts this among "the five bugs C has been
...
The `proven_println("{}", PROVEN_ARG(x))` used in chapter 75 is exactly this
...
value itself (exactly chapter 43's tagged union).
...
#qa[
  ...
  stamps code out with macros (chapter 76's container macros) or passes `void *`
  with a size (chapter 69's `qsort`).
]
...
#misconception[
  ...
  if there are many, take an array and a length (chapter 36's practice); if the
  kinds vary, take an explicit list of values with chapter 43's tagged union. The
