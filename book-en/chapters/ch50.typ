...
#prereq(
  ...
  ([chapter 42, Safe input], [five disciplines for handling failure]),
)
...
#idx("contract")  The seed of the contract planted in chapter 33 grows — what a
  ...
  that value to be checked. The five disciplines set out in chapter 42 are
...
- *A success/failure boolean* plus receiving the result through a pointer
  (chapter 35's `&` idiom).
- *A special value* marking failure — `malloc`'s null (chapter 44), `fgets`'s
...
#demo("examples-en/ch50/errval.c")
...
That is exactly why chapter 42's parsing function wears this notation.
...
#idx("const")chapter 45 as the mark "this function does not touch the original."
...
outright (chapter 41's string literals lived in that place).
...
as a `const` pointer instead of by value (chapter 45) is both safe and fast.
...
#qa[
  How do chapter 42's five disciplines connect to this principle?
][
  ...
  violations seen in chapters 37–41. In summary: a design that writes the contract
