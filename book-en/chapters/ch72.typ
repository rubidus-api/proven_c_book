...
#prereq(
  ([chapter 50, Several files], [several files and linking]),
  ([chapter 71, The five bugs shipped for fifty years], [the five bugs]),
)

#deepqa[
  Chapter 50 made a multi-file program and learned about headers, object files and
#idx("the compilation process")  linking, and chapter 16 saw the four runners of
  ...
  *definition* — exactly chapter 50's picture. proven took the former road, and the
  next section is why.
]
...
#organizer[
  ...
  chapter 71 (format mismatch) already disappears in this first program. Then we follow
...
headers (chapter 50), `-lm` joins the mathematical functions. These two lines are
...
- So chapter 71's third bug — the mismatch of format and argument — *structurally*
...
and precision seen in chapter 55. `>` is right alignment, `<` left alignment, `.3` is
...
+ *Only a function that takes an allocator as an argument takes memory.* If
  ...
  embedded work and which are not divide before your eyes (chapter 75).
...
+ *Give a thing back with the allocator you made it with.* What was obtained with
  ...
  is not destroyed (chapters 74 and 75).
...
whether to give it the heap or an arena (chapter 75).
...
take `value` out — that order is the whole of chapter 73.
...
chapter 73's `goto` cleanup idiom.
...
#antipattern[
  ...
  your hand. ③ in particular is met again in chapter 76, and once more when an arena is
  reset.
]
...
#qa[
  ...
  (chapter 76), and several of this book's examples run so.

  There is a middle form too. Take the memory once in a large piece, lay an arena over
  it and hand out from there (chapter 75) — then `malloc` is never called once while the
...
(chapter 50). *`platform/`* is the thin layer that calls the operating system, so when
going to bare metal only this line is removed (chapter 80). *`-lm`* joins the
mathematical functions that real-number formatting uses — take reals out of the
formatter (chapter 80's `PROVEN_FMT_NO_FLOAT`) and this is not needed either.
...
#platform[
  ...
  *Embedded* — leave out `platform/` and compile only `src/proven/*.c`. There being no
  ...
  laid over a static array is used instead (chapter 75). The detailed procedure is
  chapter 80.
]
