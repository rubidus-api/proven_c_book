...
#prereq(
  ([chapter 66, Character classification], [that the judgement depends on the locale]),
  ([chapter 52, The three faces of `main`], [environment variables]),
)

#deepqa[
  Chapter 66 said the answer `isalpha` gives depends on "the current locale". But
...
#demo("examples-en/ch67/locale_probe.c")
...
#dtable(
  ...
  [`LC_CTYPE`], [Classification, case, *multibyte conversion*], [`isalpha` family (chapter 66), `mbrtowc` family (chapter 69)],
...
comes from that line (chapter 68).
...
#qa[
  ...
  *numbers a machine will read and write are pinned* so the locale cannot move
  them. These two lines prevent most of the data corruption seen in chapter 68.
...
#realcase[
  ...
  in Korea, and it is also why the three layers of chapter 71 — bytes, code
