...
#prereq(
  ([chapter 66, Locales ①], [categories and `setlocale`]),
  ([chapter 61, Streams in practice], [`printf` formats]),
  ([chapter 72, Time], [`strftime`]),
)

#deepqa[
  Chapter 66 said a locale is data holding conventions. What exactly is that data
...
#qa[
  ...
  *There is none in standard C.* `localeconv` hands over the materials; the
  ...
  (chapter 48). Keep it as an integer number of the smallest unit and insert the
...
#realcase[
  ...
  `"C"` with chapter 66's idiom, and format separately when showing a person.
]
...
The time arithmetic itself has nothing to do with the locale (chapter 72). What
the locale changes is *the writing*, and the window is `strftime`'s conversion
specifiers.

#demo("examples-en/ch67/time_locale.c")
...
#demo("examples-en/ch67/collate.c")
...
Chapter 66 said the locale is process-global. In a program with several threads
