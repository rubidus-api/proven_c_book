...
#prereq(
  ([chapter 85, Errors are values], [returning failure as a value]),
  ([chapter 86, Foundations — byte, view, checked arithmetic], [borrowed slices and overflow checks]),
  ([chapter 87, Allocation — allocators, arenas, pools], [where memory comes from]),
  ([chapter 88, Strings], [refuse rather than truncate]),
)
...
#demo("examples-en/ch93/json_plain.c")
...
#demo("examples-en/ch93/json_proven.c")
...
slice pointing into the source buffer (chapter 86). With no container there is
...
(chapter 87). It does not know where the memory comes from and does not need to.
...
the stack gives way. It is chapter 42 exactly — the stack is narrow (a few MiB
...
#demo("examples-en/ch93/json_nested.c")
...
#dtable(
  ...
  [Arena (chapter 87)], [Takes the memory of one parse in a lump and drops it in a lump], [Every node needs a matching `free`],
  [Pool (chapter 87)], [Recycles slots of exactly one `jnode`], [Same-size allocations fragment the heap],
  [Intrusive list (chapter 86)], [Hooks a child onto its parent — through a link inside the node], [A separate child array must be allocated and grown],
  [Dynamic array], [Stacks the open containers — the *explicit stack*], [You end up leaning on the call stack (that is, recursion)],
  [`view` (chapter 86)], [Borrows keys and strings from the source], [Every character needs a copy and a container],
  [Checked arithmetic (chapter 86)], [Watches overflow at every carry], [One forgotten `errno` and it wraps],
  [`proven_err_t` (chapter 85)], [Depth exceeded, no room, bad syntax — all as values], [Either death, or a silent trim],
)
...
#qa[
  ...
  That is why chapter 83's five bugs have been shipping for half a century. Not
