...
#prereq(
  ([chapter 52, Several files], [a project of several files]),
  ([chapter 17, Setting up a development environment], [setting up the tools]),
)

#deepqa[
  Chapter 52 said that for a multi-file program the practice is to make each file
...
chapter 16's relay and chapter 52's division save time in practice.
...
#dtable(
  ...
  [`massif`, `heaptrack`], [heap usage over time], [leaks and over-allocation (chapter 43)],
...
expensive allocations (chapter 43) are crowded, all become visible as they are.
...
#platform[
  ...
  - *Measuring stack usage* — the stack painting seen in chapter 80, and static
...
grey areas and chapter 50's undefined behaviour are handled in practice is written
...
#dtable(
  ...
  [Dynamic allocation (`malloc`/`free`)], [Fragmentation and unpredictable timing (chapters 43, 81); arenas and pools instead],
  [Recursion], [You cannot know how much stack it will take (chapter 80)],
  [Code that relies on undefined behaviour], [The whole of chapter 50],
  [Expressions that lean on implicit conversion], [Chapter 29 — explicit casts are demanded],
  [`goto`, multiple `return`, multiple `break`], [Making the flow readable at a glance (chapter 31)],
  [Variadic functions], [No type checking (chapter 56)],
  [Type punning through a union], [Code that leans on representation (chapter 46)],
  [Overuse of `#define` macros], [The preprocessor knows no types (chapter 55)],
  [Parts of the standard library], [Things like `gets`, `atoi`, `strcpy` (chapters 62, 63)],
)
...
#dtable(
  ...
  [variable-length arrays (chapter 38)], [templates and the standard library upon them],
  [flexible array members], [references (`&`) and operator overloading],
  [`_Generic` (chapter 56)], [exceptions and RAII],
  [order-independent designated initialisers], [namespaces],
  [compound literals (chapter 45)], [`auto` type deduction, lambdas],
)
...
#demo("examples-en/ch93/ccompat.c")
...
function name it plants in the object file (recall chapter 52's linking), while a C
...
the standard library (chapter 59's freestanding) and the code generated is predictable.
...
and 28), not using dynamic allocation at all (chapter 43), handling communication
protocols conscious of alignment and representation (chapters 6 and 46).
...
- *No namespaces* → the prefix convention, file-level `static` and symbol
  visibility (chapters 53 and 54 — strictly, the standard defines four name spaces;
  what you cannot do is *make a new one*).
- *No generics* → code generation with macros (chapter 55's `##`, the X macro), or
...
- *Resource cleanup is not automatic* → state the ownership convention in names and
  documentation (chapter 43), and gather one function's cleanup points into one place
...
- *The complexity of large projects* → divide the translation units small and reveal the
  contracts through headers (chapter 52), and automate the tests and the build (this
...
#qa[
  ...
  statistics, a simple configuration-file parser (chapter 88's scanner is used as it
  stands), implementing one data structure yourself (chapter 44's linked structures),
