...
#prereq(
  ([chapter 50, Undefined behaviour], [undefined behaviour]),
  ([chapter 49, Errors and contracts], [contracts]),
  ([chapter 93, C in practice], [the tools of practice]),
)
...
#deepqa[
  ...
  (chapter 49), avoid shortcuts outside the contract (chapter 50), check with tools
  (chapter 17). C23's new syntax appeared as tools that make that criterion *easier to
  keep*. The retrospect below is that list.
]

#organizer[
  The last chapter. Chapter 93 looked round the terrain of practice, so now we gather
...
#dtable(
  ...
  [`[[nodiscard]]`], [chapters 37 and 49], [the compiler blocks the mistake of forgetting to confirm failure],
  [`[[fallthrough]]`], [chapter 31], [the intent of switch fallthrough as documentation],
  [the `ckd_add` family], [chapter 50], [overflow as a value rather than UB],
  ...
  [the relaxation of `va_start`], [chapter 56], [removing the excess of variadic arguments],
)
...
#qa[
  ...
  to: data structures and algorithms (chapter 44's linked structures), concurrency
  ...
  directly the source of the projects named in chapter 93. Well-written C code like
