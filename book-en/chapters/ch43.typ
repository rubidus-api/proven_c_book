...
#prereq(
  ([chapter 42, Structs], [defining a struct]),
  ([chapter 37, Arrays], [arrays and passing by value]),
)

#deepqa[
  Chapter 42 said a struct is a value, so assigning copies it whole and passing it
...
#organizer[
  If chapter 42 was the syntax of the struct, this chapter is *how to use it*.
...
`p->x` is an abbreviation of `(*p).x` (chapter 42), and the abbreviation exists
...
#antipattern[
  ...
  allocation (chapter 41). A compound literal written at file scope has static
...
#realcase[
  ...
  `struct sigaction`, chapter 64, or `struct timespec`), and the `..._desc`
...
#demo("examples-en/ch43/layout.c")
...
#misconception[
  ...
  - *Do not compare structs with `memcmp`* (chapter 59) — equal values may come
...
standard word of C23 (chapter 68).
...
#qa[
  ...
  type runs into the aliasing rules (chapter 48). Field-by-field `memcpy` is longer
...
smaller in recursion or on a per-thread stack (chapter 65). Recursion passing
