...
#deepqa[
  ...
  chapter 42 used the phrase "the return ledger of a function call". Are a name
...
#dtable(
  ...
  [allocated], [`malloc` ~ `free`], [chapter 45],
)
...
same object*. It is the groundwork of chapter 53 (several files).
...
- *`typedef`* — syntactically it goes in this slot but does something entirely
  ...
  chapter 59).
- *`thread_local`* — makes an object that is separate per thread (chapter 77). It
...
#qa[
  ...
  definition in one source file comes from here (chapter 53).
]
...
#demo("examples-en/ch43/regions.c")
...
#dtable(
  ...
  [heap], [what `malloc` gave (chapter 45)], [until freed],
...
#platform[
  ...
  *The C standard has neither the word "stack" nor any promise about its size.* It
  ...
  work, are treated in chapter 81.
]
...
Second, why chapter 42's boundary-violation attack is so dangerous — overflow an
...
#qa[
  ...
  the subject of chapter 53 (linkage). And the practical advice is an old one:
...
settled at run time and which stays alive as long as you wish — chapter 44's
dynamic memory.
