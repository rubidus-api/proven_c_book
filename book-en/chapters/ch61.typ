...
#prereq(
  ([chapter 60, The terrain of the standard library], [the character of the standard library]),
)

#deepqa[
  Chapter 60 said the standard library is "thin and old", and that the standard
...
#organizer[
  ...
  those regions. It is the chapter that redraws chapter 60's terrain map at a
  larger scale.
]
...
#dtable(
  ...
  [`<stdarg.h>`], [variadic arguments], [chapter 57],
...
#dtable(
  ...
  [`<assert.h>`], [C89], [`assert` — the diagnosis that catches contract violations], [chapter 74],
  [`<complex.h>`], [C99], [complex arithmetic], [chapter 72],
  [`<ctype.h>`], [C89], [character classification and conversion], [chapter 66],
  [`<errno.h>`], [C89], [the error-number global], [chapter 74],
  [`<fenv.h>`], [C99], [the floating-point environment (rounding, exceptions)], [chapter 72],
  [`<float.h>`], [C89], [the limits of real types], [chapters 27, 72],
  [`<inttypes.h>`], [C99], [formats and conversions for fixed-width integers], [appendix B, chapter 77],
  [`<iso646.h>`], [C95], [alternative spellings of operators], [chapter 77],
  [`<limits.h>`], [C89], [the limits of integer types], [chapter 27],
  [`<locale.h>`], [C89], [locale settings], [chapter 66],
  [`<math.h>`], [C89], [mathematical functions], [chapter 72],
  [`<setjmp.h>`], [C89], [non-local jumps], [chapter 74],
  [`<signal.h>`], [C89], [signal handling], [chapter 74],
  [`<stdalign.h>`], [C11], [specifying and querying alignment], [chapter 77],
  [`<stdarg.h>`], [C89], [variadic arguments], [chapter 57],
  [`<stdatomic.h>`], [C11], [atomic operations], [chapter 78],
  [`<stdbit.h>`], [C23], [bit manipulation (counting, rotating and so on)], [chapter 77],
  [`<stdbool.h>`], [C99], [`bool`, `true`, `false`], [chapter 77],
  [`<stdckdint.h>`], [C23], [arithmetic that checks for overflow], [chapters 51, 79],
  [`<stddef.h>`], [C89], [`size_t`, `ptrdiff_t`, `NULL`, `offsetof`], [chapter 77],
  [`<stdint.h>`], [C99], [fixed-width integer types], [chapters 27, 77],
  [`<stdio.h>`], [C89], [stream input and output, files], [chapters 53, 63],
  [`<stdlib.h>`], [C89], [conversion, random numbers, allocation, sorting, program termination], [chapter 65],
  [`<stdnoreturn.h>`], [C11], [`noreturn`], [chapter 77],
  [`<string.h>`], [C89], [strings and memory blocks], [chapter 64],
  [`<tgmath.h>`], [C99], [type-generic mathematical functions], [chapter 72],
  [`<threads.h>`], [C11], [threads, mutexes, condition variables], [chapter 77],
  [`<time.h>`], [C89], [time and the calendar], [chapter 73],
  [`<uchar.h>`], [C11], [UTF-16 and UTF-32 character types], [chapter 66],
  [`<wchar.h>`], [C95], [wide-character input, output and strings], [chapter 66],
  [`<wctype.h>`], [C95], [wide-character classification], [chapter 66],
)
...
#misconception[
  ...
  the 1989 standard and was deleted only in 2011 (chapter 63). `strncpy`, contrary
  to its name, is not a safe copying function (chapter 64), and `atoi` has no way
  at all to report failure (chapter 65). Some functions of *the same name even
  behave differently according to the locale* (chapter 66). Using the standard
