...
#prereq(
  ([chapter 45, Structs], [defining a struct]),
  ([chapter 38, Arrays], [arrays and passing by value]),
)

#deepqa[
  Chapter 45 said a struct is a value, so assigning copies it whole and passing it
...
#organizer[
  If chapter 45 was the syntax of the struct, this chapter is *how to use it*.
...
`p->x` is an abbreviation of `(*p).x` (chapter 45), and the abbreviation exists
...
#antipattern[
  ...
  allocation (chapter 44). A compound literal written at file scope has static
...
#realcase[
  ...
  `struct sigaction`, chapter 74, or `struct timespec`), and the `..._desc`
...
#demo("examples-en/ch46/layout.c")

Chapter 45 gave the three rules of padding — each member at a multiple of its own
...
#misconception[
  ...
  - *Do not compare structs with `memcmp`* (chapter 64) — equal values may come
...
#dtable(
  ...
  [Byte order (endianness) differs], [`0x01020304` becomes `0x04030201` at the other end (chapter 47)],
  [Type sizes differ], [`long`, `size_t`, pointers and enums differ between 32- and 64-bit],
)

#demo("examples-en/ch46/serialize.c")
...
#realcase[
  ...
  layer is settled explicitly. Chapter 45's `{ 0 }` initialiser does the same job
  more safely.
...
#qa[
  ...
  and fix them by eye. That is also why chapter 88 writes a small JSON reader
  three ways.
...
standard word of C23 (chapter 80).
...
#qa[
  ...
  type runs into the aliasing rules (chapter 51). Field-by-field `memcpy` is longer
...
#demo("examples-en/ch46/container_of.c")
...
A tagged union (chapter 47) reads better for it — the `u` in `t.u.i` never meant
...
smaller in recursion or on a per-thread stack (chapter 77). Recursion passing
