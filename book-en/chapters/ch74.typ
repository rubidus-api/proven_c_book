...
#prereq(
  ...
  ([chapter 72, The five bugs shipped for fifty years], [the unchecked return value]),
)
...
#deepqa[
  ...
  `-1` — that trap seen in chapter 72), take the result out as an *output parameter*
...
#organizer[
  We see the answer to chapter 72's second bug — unconfirmed failure. The way of
...
#dtable(
  ...
  [`proven_result_mem_mut_t`], [`proven_mem_mut_t`], [allocators (chapters 75 and 76)],
  [`proven_result_mem_view_t`], [`proven_mem_view_t`], [slicing (chapter 75)],
  [`proven_result_u8str_t`], [`proven_u8str_t`], [making a string (chapter 77)],
  [`proven_result_buf_t`], [`proven_buf_t`], [making a buffer],
  [`proven_result_cstr_t`], [`const char *`], [exporting as a C string (chapter 77)],
  [`proven_fmt_result_t`], [(amount written and amount needed)], [formatting (chapter 78)],
)
...
it carries two numbers as well (we look at it closely in chapter 78).

#demo("examples-en/ch74/errval.c")
...
exact opposite choice from `snprintf`'s quiet truncation seen in chapter 72.

== Throw it away and the compiler protests

Returning the error as a value is not enough by itself. As seen in chapter 72, a
...
#qa[
  But chapter 73's `proven_println` had no such mark. Why is screen output alone an
  exception?
][
  ...
  read, replaying chapter 72's second bug exactly. Where to attach the mark is itself
  a design judgement.
]
...
#demo("examples-en/ch74/cleanup.c")
...
#qa[
  ...
  As chapter 76 will show, an allocator is simply a value, so a shell that "fails from
...
usable in embedded work too (chapter 81).
...
functions with `_or_panic` in the name (chapter 76's arena allocation is
...
#realcase[
  ...
  *the error paths must be visible in the shape of the code*. Exceptions are
  ...
  chapter 72's table from the same direction.
]
