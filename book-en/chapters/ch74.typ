...
#prereq(
  ...
  ([chapter 67, How to ask about overflow], [arithmetic that does not overflow]),
)
...
#idx("view")  points at raw bytes, the *view* binding pointer and length into one,
  size calculation that does not wrap, and alignment. Chapter 71's sixth bug (the
...
the answer to the problem chapter 38 called "the real problem of strings is carrying
the length separately".

#dtable(
  columns: 4,
  [*type*], [*owns*], [*writes*], [*where it comes from*],
  [`proven_mem_t`], [yes], [yes], [an allocator (chapter 75)],
...
overlap it is `proven_mem_move` — chapter 59's `memcpy`/`memmove` distinction as it
stands.
...
`uintptr_t` and then checks. It is the function chapter 75's arena uses when confirming
...
support range in chapter 80).

== Slicing — the operation used most in this part

#demo("examples-en/ch74/view.c")
...
was requested. It is blocking chapter 71's truncation problem from repeating here.
...
#qa[
  ...
  chapter 47's "write the contract as code", with the practical benefit too that
...
#antipattern[
  ...
  use after that is the access to a dead automatic variable learned in chapter 40.
...
does not know what type is coming* uses `PROVEN_MAX_ALIGN`. It is also why chapter 75's
