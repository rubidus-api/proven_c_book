...
#prereq(
  ...
  ([chapter 74, The five bugs shipped for fifty years], [the five bugs]),
)
...
#organizer[
  ...
  chapter 74 (format mismatch) already disappears in this first program. Then we follow
...
- So chapter 74's third bug — the mismatch of format and argument — *structurally*
...
+ *Only a function that takes an allocator as an argument takes memory.* If
  ...
  embedded work and which are not divide before your eyes (chapter 78).
...
+ *Give a thing back with the allocator you made it with.* What was obtained with
  ...
  is not destroyed (chapters 77 and 78).
...
whether to give it the heap or an arena (chapter 78).
...
take `value` out — that order is the whole of chapter 76.
...
chapter 76's `goto` cleanup idiom.
...
#antipattern[
  ...
  your hand. ③ in particular is met again in chapter 79, and once more when an arena is
  reset.
]
...
#qa[
  ...
  (chapter 79), and several of this book's examples run so.

  There is a middle form too. Take the memory once in a large piece, lay an arena over
  it and hand out from there (chapter 78) — then `malloc` is never called once while the
...
To avoid typing the two lines above every time, use chapter 85's `make`. Supposing the
...
going to bare metal only this line is removed (chapter 83). *`-lm`* joins the
mathematical functions that real-number formatting uses — take reals out of the
formatter (chapter 83's `PROVEN_FMT_NO_FLOAT`) and this is not needed either.
...
#platform[
  ...
  *Embedded* — leave out `platform/` and compile only `src/proven/*.c`. There being no
  ...
  laid over a static array is used instead (chapter 78). The detailed procedure is
  chapter 83.
]
