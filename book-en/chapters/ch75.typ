...
#prereq(
  ...
  ([chapter 73, The five bugs shipped for fifty years], [the unchecked return value]),
)
...
#deepqa[
  ...
  `-1` — that trap seen in chapter 73), take the result out as an *output parameter*
...
#organizer[
  We see the answer to chapter 73's second bug — unconfirmed failure. The way of
...
#dtable(
  ...
  [`proven_result_mem_mut_t`], [`proven_mem_mut_t`], [allocators (chapters 76 and 77)],
  [`proven_result_mem_view_t`], [`proven_mem_view_t`], [slicing (chapter 76)],
  [`proven_result_u8str_t`], [`proven_u8str_t`], [making a string (chapter 78)],
  [`proven_result_buf_t`], [`proven_buf_t`], [making a buffer],
  [`proven_result_cstr_t`], [`const char *`], [exporting as a C string (chapter 78)],
  [`proven_fmt_result_t`], [(amount written and amount needed)], [formatting (chapter 79)],
)
...
it carries two numbers as well (we look at it closely in chapter 79).

#demo("examples-en/ch75/errval.c")
...
exact opposite choice from `snprintf`'s quiet truncation seen in chapter 73.

== Throw it away and the compiler protests

Returning the error as a value is not enough by itself. As seen in chapter 73, a
...
#qa[
  But chapter 74's `proven_println` had no such mark. Why is screen output alone an
  exception?
][
  ...
  read, replaying chapter 73's second bug exactly. Where to attach the mark is itself
  a design judgement.
]
...
#demo("examples-en/ch75/cleanup.c")
...
#qa[
  ...
  As chapter 77 will show, an allocator is simply a value, so a shell that "fails from
...
usable in embedded work too (chapter 82).
...
functions with `_or_panic` in the name (chapter 77's arena allocation is
...
#realcase[
  ...
  *the error paths must be visible in the shape of the code*. Exceptions are
  ...
  chapter 73's table from the same direction.
]
