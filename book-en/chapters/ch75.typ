...
#prereq(
  ([chapter 74, Diagnostics and control], [`errno` and the handler's restrictions]),
  ([chapter 52, The three faces of `main`], [a program's start and end]),
  ([chapter 3, Programs and processes], [the process as a container]),
)

#deepqa[
  Chapter 74 said a signal handler may not call `printf` or `malloc`, and that
...
#qa[
  ...
  operating system* (chapter 61). `sigaction` stands on operating-system notions —
...
Why `signal`'s declaration is rough was read by procedure in chapter 59 — *"a
...
if there is no handler or the handler returns (chapter 66).

#demo("examples-en/ch75/sig_basic.c")
...
#dtable(
  ...
  [Handling lock-free atomic objects], [`<stdatomic.h>`, when lock-free (chapter 78)],
  [Calling `abort`], [—],
  [Calling `_Exit`], [—],
  [Calling `quick_exit`], [—],
...
handler (chapter 78). In a program with several threads that is the more accurate
...
An allocator manages a data structure called the *free list* (chapter 44). One
...
(chapter 76). Here the trouble comes without calling `malloc` again — because you
*jump out still holding the lock*. From then on the main flow's first `malloc`
hangs. Half the reason chapter 76 calls jumping from a handler dangerous is this.
...
#demo("examples-en/ch75/sig_context.c")
...
#demo("examples-en/ch75/sig_flag.c")
...
#demo("examples-en/ch75/sig_action.c")
...
`sa_handler` and `sa_sigaction` usually *overlap in a union* (chapter 46). So only
...
Ignored, `write` returns the failure *as a value* (`errno == EPIPE`). Chapter 74's
