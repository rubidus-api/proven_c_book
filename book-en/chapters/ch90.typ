...
#prereq(
  ([chapter 61, Streams in reality], [streams]),
  ([chapter 25, Input], [input]),
)

#deepqa[
  Chapter 10's design had a stream be one where "the program does not know what it is
  connected to", and chapter 59 said `fopen` reports failure with null. Then what
...
#demo("examples-en/ch90/fslife.c")
...
#demo("examples-en/ch90/fileio.c")

Several things stand out.

*The path is a view too.* `proven_u8str_view_from_cstr("...")` — wherever the string
came from, it is handled as a pointer and a length (chapter 87).
...
system requires. Chapter 86's rule is honestly kept here too — *if it can allocate, it
takes an allocator*.

*What is read becomes a view.* Bind the buffer and the number of bytes read and from
then on all of chapter 87's tools can be used. That is what the example used to divide
...
Date formatting uses the format syntax seen in chapter 59 as it is — named
