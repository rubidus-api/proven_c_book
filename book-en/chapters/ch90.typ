...
#platform[
  ...
  - *Measuring stack usage* — the stack painting seen in chapter 77, and static
...
#dtable(
  ...
  [`_Generic` (chapter 53)], [exceptions and RAII],
...
#demo("examples-en/ch90/ccompat.c")
...
the standard library (chapter 56's freestanding) and the code generated is predictable.
...
- *No namespaces* → the prefix convention (chapter 40's `proven_`) and file-level
  `static` (chapter 51's internal linkage).
- *No generics* → code generation with macros (chapter 52's `##`, the X macro), or
...
#qa[
  ...
  statistics, a simple configuration-file parser (chapter 85's scanner is used as it
