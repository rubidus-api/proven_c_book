...
#prereq(
  ...
  ([chapter 79, How to ask about overflow], [arithmetic that does not overflow]),
)
...
#idx("view")  points at raw bytes, the *view* binding pointer and length into one,
  size calculation that does not wrap, and alignment. Chapter 83's sixth bug (the
...
the answer to the problem chapter 41 called "the real problem of strings is carrying
the length separately".

#dtable(
  columns: 4,
  [*type*], [*owns*], [*writes*], [*where it comes from*],
  [`proven_mem_t`], [yes], [yes], [an allocator (chapter 87)],
...
overlap it is `proven_mem_move` — chapter 64's `memcpy`/`memmove` distinction as it
stands.
...
`uintptr_t` and then checks. It is the function chapter 87's arena uses when confirming
...
support range in chapter 92).

== Slicing — the operation used most in this part

#demo("examples-en/ch86/view.c")
...
was requested. It is blocking chapter 83's truncation problem from repeating here.
...
#qa[
  ...
  chapter 50's "write the contract as code", with the practical benefit too that
...
#antipattern[
  ...
  use after that is the access to a dead automatic variable learned in chapter 43.
...
does not know what type is coming* uses `PROVEN_MAX_ALIGN`. It is also why chapter 87's
