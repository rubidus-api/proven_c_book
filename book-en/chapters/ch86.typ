...
#prereq(
  ([chapter 43, Dynamic memory], [dynamic memory]),
  ([chapter 81, Inside the allocator], [inside the allocator]),
)

#deepqa[
  Chapter 43 said memory obtained with `malloc` must be `free`d by somebody exactly
...
#organizer[
  The answer to chapter 82's fourth bug — unclear ownership. The *allocator* that
...
#idx("arena")  things of the same lifetime all at once, and the rule that divides
  owning from borrowing by type. The dangers of the heap learned in chapter 43 are
...
and returned from a function. The *virtual function table* seen in chapter 57 is here
as it stands.
...
— because the allocator wrote it into the block's header (chapter 81). But an arena has
...
+ *`align` must be a power of two.* And *a block must be reallocated and freed with the
  ...
  or below the default alignment differently from stricter ones (chapter 85) — handing
...
+ *Reallocation is failure-atomic.* On failure `old_ptr` is still valid and its contents
  untouched — chapter 64's `realloc` leak counterexample blocked at the level of the
  interface.
...
#qa[
  ...
  Of course. Chapter 84's example already did — a testing allocator that "fails from the
  ...
  *Special resources* — obtaining from shared memory or a DMA-capable region, places
  ...
  allocator inside, do their own work and pass it on. Chapter 84's example is the model.
]
...
#qa[
  ...
  with no heap* — such is embedded work (chapter 91), and such is inside a kernel.
...
Chapter 43 taught the heap's three dangers (leak, double free, use after free). All
...
The `_or_panic` family exists for the reason given in chapter 84 — where the memory
...
chapter 87's `proven_u8str_reserve` in advance pays especially on an arena (an
intervening allocation breaks the property).

#demo("examples-en/ch86/arena.c")
...
#misconception[
  ...
  still be alive after the reset must not be put in the arena. Chapter 85's "a view
...
#dtable(
  ...
  [data of assorted lifetimes], [the heap], [general-purpose. individual release possible], [cost and fragmentation (chapter 81)],
  ...
  [testing the out-of-memory path], [a failing shell (chapter 84)], [it runs the path that never otherwise runs], [for testing only],
)
...
Now we return to chapter 82's fourth bug. The problem of `char *` meaning four things
...
Chapter 44 taught that struct assignment is a shallow copy. Copy an object that holds
