...
#organizer[
  The answer to chapter 81's first bug — string functions that do not know the size
...
The rule set up in chapter 85 applied to strings as it is. `u8` means UTF-8 — that
...
```c
/* the borrowed string — the same shape as chapter 84's mem_view_t */
typedef struct {
...
#demo("examples-en/ch86/ops.c")

The first line shows chapter 85's rule again. `proven_u8str_borrow` has no allocator —
...
It is the exact opposite of `snprintf`'s choice seen in chapter 81. Why not truncate?
...
#qa[
  ...
  name (the same as chapter 84's `_unchecked`) is kept here too.
]
...
`_reserve`. As seen in chapter 85, `_reserve` pays especially on an arena — take it
...
kept — as seen in chapter 84, *it is valid only while the original is alive*.
...
Chapter 81 spoke of the danger of sentinel values, and what differs here is that *it
...
the place where chapter 81's third bug waits, and the place where this library's most
