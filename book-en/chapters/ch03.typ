...
#dtable(
  ...
  [address space], [a memory map of its own (chapter 2's four regions)], [chapter 80],
  [execution position], [which instruction it is in the middle of], [—],
  [the list of open files], [the passages including standard input and output], [chapters 10 and 61],
  [command-line arguments and environment], [the values handed over at running], [chapter 51],
  [exit status], [the one number it leaves as it ends], [chapters 15 and 51],
)
...
#qa[
  ...
  the whole machine (chapter 80). So this chapter's story is not the story of
...
#misconception[
  ...
  (chapter 60's freestanding implementation). C must exist on chips that have no
...
#qa[
  ...
  *one within one process*.
  ...
  C standard treats are met in their places (signals in chapter 73, threads and atomic
  operations in chapter 77).
]
