/* Same data, same condition --- only the order differs. Ask the machine about the gap.
   * This example measures time. No absolute times, only ratios (they vary by run). */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <time.h>

#define N      (1u << 20)     /* one million elements */
#define ROUNDS 5
...
/* Add only the values that pass the test --- whether that test is easy to predict
...
int main(void)
{
    ...
    sum_over(shuffled); sum_over(sorted);                    /* warm-up */
