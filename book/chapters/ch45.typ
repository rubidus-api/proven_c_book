...
(47장), `va_arg(ap, float)`라고 쓰면 도착하지도 않은 타입을 꺼내는
계약 위반이 된다.
...
검사해 줄 정보가 애초에 존재하지 않기 때문이다. 61장에서 이것을 "C가
...
67장에서 쓴 `proven_println("{}", PROVEN_ARG(x))`가 정확히 이 구조다.
...
#qa[
  ...
  필요하면 C에서는 여전히 매크로로 코드를 찍어 내거나(68장의 컨테이너
  매크로), `void *`와 크기를 넘기는 방식(61장의 `qsort`)을 쓴다.
]
