...
#prereq(
  ([22장 출력], [printf 라는 가변 인자 함수]),
  ([29장 암묵적 변환], [기본 인자 승격]),
)

#deepqa[
#idx("가변 인자")  29장에서 가변 인자로 넘어가는 값에는 기본 진급(float→double,
  작은 정수→int)이 적용된다고 배웠다. 그런데 왜 하필 그 자리에만 특별한
  진급 규칙이 있는가?
][
  받는 쪽이 *타입을 모르기 때문이다.* 보통의 함수는 원형(24장)에 인자
...
#organizer[
  ...
  에서 C89의 `stdarg`, C23의 정리까지), 그리고 오늘의 대안까지. 22장의
...
밖이다(51장). 기억의 잘못된 자리를 읽는 것이니 값이 쓰레기가 되거나
그 뒤의 꺼내기가 전부 어긋난다.
...
타입을 동시에 알려 주는 명세*인 것이다. 22장에서 "서식은 계약"이라 한
말의 정확한 무게가 이것이다 — 그 계약이 깨지면 printf는 있지도 않은
인자를 꺼내며 메모리를 뒤진다(22장의 서식 문자열 취약점이 바로 이
메커니즘이었다).

== 기본 인자 승격 — `...`을 건너는 값은 굵어진다

`va_arg`가 타입을 물어보는 이유가 하나 더 있다. `...`을 건너는 인자는
*원래 타입 그대로 가지 않기* 때문이다. 29장에서 본 기본 인자
...
(60장), `va_arg(ap, float)`라고 쓰면 도착하지도 않은 타입을 꺼내는
계약 위반이 된다.

표의 별표에 단서가 하나 붙는다. 정수 승격의 결과는 *거의 언제나* `int`
이지만, 정확히는 "`int`가 그 타입의 모든 값을 표현할 수 있으면 `int`,
아니면 `unsigned int`"다(29장). 오늘의 주류 기계에서는 `unsigned short`도
...
`va_arg(ap, char*)`로 꺼내면 폭이 어긋난다. C23의 `nullptr`(36장)이나
...
검사해 줄 정보가 애초에 존재하지 않기 때문이다. 83장에서 이것을 "C가
...
89장에서 쓴 `proven_println("{}", PROVEN_ARG(x))`가 정확히 이 구조다.
...
#qa[
  ...
  필요하면 C에서는 여전히 매크로로 코드를 찍어 내거나(90장의 컨테이너
  매크로), `void *`와 크기를 넘기는 방식(83장의 `qsort`)을 쓴다.
]
...
*표준 이전(K&R 시절).* 원형이 없던 시절(12장)의 C에서는 사실상 모든
...
*C89 — `<stdarg.h>`.* 표준화 위원회가 원형을 도입하면서(10·24장) 가변
...
가변 인자의 정체를 알았다 — 함수에 *값*을 여럿 넘기는 법이었다. 다음
장은 그 거울상이다. 함수 자체를 값으로 다루는 법, 즉 함수 포인터다.
