...
밖이다(50장). 기억의 잘못된 자리를 읽는 것이니 값이 쓰레기가 되거나
그 뒤의 꺼내기가 전부 어긋난다.
...
(59장), `va_arg(ap, float)`라고 쓰면 도착하지도 않은 타입을 꺼내는
계약 위반이 된다.
...
검사해 줄 정보가 애초에 존재하지 않기 때문이다. 82장에서 이것을 "C가
...
88장에서 쓴 `proven_println("{}", PROVEN_ARG(x))`가 정확히 이 구조다.
...
#qa[
  ...
  필요하면 C에서는 여전히 매크로로 코드를 찍어 내거나(89장의 컨테이너
  매크로), `void *`와 크기를 넘기는 방식(82장의 `qsort`)을 쓴다.
]
...
#misconception[
  ...
  *특별히* 서식 문자열을 읽어 검사해 주기 때문이지(17장의 경고), 언어의
  ...
  이면 배열과 길이를 받고(38장의 관행), 종류가 다양하면 46장의 태그된
