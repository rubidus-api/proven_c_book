...
#organizer[
  ...
  에서 C89의 `stdarg`, C23의 정리까지), 그리고 오늘의 대안까지. 23장의
...
#idx("가변 인자")  29장에서 가변 인자로 넘어가는 값에는 기본 진급(float→double,
  ...
  받는 쪽이 *타입을 모르기 때문이다.* 보통의 함수는 원형(25장)에 인자
...
밖이다(47장). 기억의 잘못된 자리를 읽는 것이니 값이 쓰레기가 되거나
그 뒤의 꺼내기가 전부 어긋난다.
...
타입을 동시에 알려 주는 명세*인 것이다. 23장에서 "서식은 계약"이라 한
말의 정확한 무게가 이것이다 — 그 계약이 깨지면 printf는 있지도 않은
인자를 꺼내며 메모리를 뒤진다(23장의 서식 문자열 취약점이 바로 이
메커니즘이었다).
...
(54장), `va_arg(ap, float)`라고 쓰면 도착하지도 않은 타입을 꺼내는
계약 위반이 된다.
...
검사해 줄 정보가 애초에 존재하지 않기 때문이다. 70장에서 이것을 "C가
...
76장에서 쓴 `proven_println("{}", PROVEN_ARG(x))`가 정확히 이 구조다.
...
#qa[
  ...
  필요하면 C에서는 여전히 매크로로 코드를 찍어 내거나(77장의 컨테이너
  매크로), `void *`와 크기를 넘기는 방식(70장의 `qsort`)을 쓴다.
]
...
*C89 — `<stdarg.h>`.* 표준화 위원회가 원형을 도입하면서(10·25장) 가변
