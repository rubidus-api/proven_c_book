...
- **48장 「`main`의 세 얼굴」** — 표준 3형태, `void main()` 반례, argc/argv
  계약, 종료 방법 표, 하위 8비트 잘림, 리눅스·윈도우·임베디드 관례.
- **40장에 「표준의 네 축」** — 저장 기간·유효범위·연결·저장 클래스 지정자,
  `static`의 두 얼굴, 선언과 정의.
- **부록 F** — EBNF 표기법과 C 문법 참고(선언자·문장·식 계층, `A * B;`).
- **80장 도구 절 전면 보강** — 빌드·의존성, 디버거와 기록·재생, 프로파일러,
  검사·시험, 서식·문서, 임베디드 관측.
- 영어판 12~19장 번역(누적: 1장, 4~19장).
...
- 반복된 재번호로 어긋나 있던 장 범위 표기(`N–M장`)를 전수 정정.
