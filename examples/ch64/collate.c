...
int main(void)
{
    ...
    puts("strxfrm 은 로케일 비교를 '키'로 굳혀 둔다:");
    for (size_t i = 0; i < sizeof locales / sizeof *locales; i++) {
        if (!setlocale(LC_ALL, locales[i])) continue;

        char key[64];
        size_t need = strxfrm(key, "Äpfel", sizeof key);
        printf("  %-12s \"Äpfel\" → 키 %zu바이트: ", locales[i], need);
        if (need >= sizeof key) { puts("(버퍼 부족)"); continue; }
        for (size_t k = 0; k < need && k < 12; k++)
            printf("%02X ", (unsigned char)key[k]);
        puts(need > 12 ? "..." : "");
    }

    puts("\n키끼리 strcmp 한 결과가 strcoll 과 같은지 확인한다:");
    for (size_t i = 0; i < sizeof locales / sizeof *locales; i++) {
        if (!setlocale(LC_ALL, locales[i])) continue;
        char ka[64], kb[64];
        ...
        printf("  %-12s strcoll %s 0, 키 strcmp %s 0 — %s\n", locales[i],
               c1 < 0 ? "<" : c1 > 0 ? ">" : "=",
               c2 < 0 ? "<" : c2 > 0 ? ">" : "=",
               ((c1 < 0) == (c2 < 0) && (c1 > 0) == (c2 > 0)) ? "같다" : "다르다");
    }
    return 0;
}
