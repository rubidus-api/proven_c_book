...
# 한국어: PDF + HTML
cp "$root/dist/proven_c_book-$ver.pdf" "$stage/ko/proven_c_book-$ver-ko.pdf"
cp "$root/docs/ko/index.html"          "$stage/ko/proven_c_book-$ver-ko.html"
# 영어: PDF + HTML (있을 때만)
[ -f "$root/build/book-en.pdf" ]  && cp "$root/build/book-en.pdf" "$stage/en/proven_c_book-$ver-en.pdf"
[ -f "$root/docs/en/index.html" ] && cp "$root/docs/en/index.html" "$stage/en/proven_c_book-$ver-en.html"
for d in ko en; do
