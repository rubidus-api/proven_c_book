...
장 서두는 다음 넷을 *이 순서로* 갖춘다.

    = N 장 제목
    #prereq(...)             ① 이 장이 기대는 것    (1장 제외)
    #deepqa[...][...]        ② 인출 문답             (1장 제외)
    #organizer[...]          ③ 이 장이 끝나면
    #chapter-questions()     ④ 이 장에서 답할 질문

순서의 근거는 학습 동선이다 — 무엇에 기대는지 알려 주고(①), 그것을 실제로
꺼내 보게 하고(②), 그 위에서 무엇을 얻을지 예고하고(③), 답할 질문을 미리
보여 준다(④). ②가 ①의 바로 뒤에 오는 것이 이 정형의 핵심이다: 선행 개념을
*표시*하는 데 그치지 않고 *인출*까지 시킨다.

검사 항목
  1. 네 장치가 각각 정확히 한 번 (1장은 ③④만).
  2. 네 장치가 정해진 순서로.
  3. 넷 모두 장 제목(`= `)보다 뒤에, 첫 절 제목(`== `)보다 앞에.
...
def main() -> int:
    ...
    if problems:
        ...
        return 1
    print("check-chapter-openings: 모든 장이 서두 정형(기댄 것 → 인출 → 목표 → 질문)을 지킨다")
    return 0
